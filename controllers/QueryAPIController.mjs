...
class QueryAPIController {
  ...
  async update_query(req, res, _next) {
  ...
  async update_user_query(req, res, _next) {
    ...
    try {
      const user_saved_data = await this._update_user_query(user_id, sid, is_deleted, (store_user_query) => {
  ...
  async delete_user_queries(req, res, _next) {
    ...
    if (!cmn.is_array(query_ids)) {
      return wutil.send_error(res, cmn.HTTP_CODE.BAD_REQUEST, `Expected body to be JSON array. Got: ${JSON.stringify(query_ids)}`);
    }
    const user_id = wutil.request_to_user_id(req);
    try {
      const _ = await this.user_service.deleteUserSaveBatch(user_id, query_ids);
  ...
  async restore_user_queries(req, res, _next) {
    const query_ids = await req.body;
    if (!cmn.is_array(query_ids)) {
      return wutil.send_error(res, cmn.HTTP_CODE.BAD_REQUEST, `Expected body to be JSON array. Got: ${JSON.stringify(query_ids)}`);
    }
    ...
    let queries = null;
    try {
      const _ = await this.user_service.restoreUserSaveBatch(user_id, query_ids);
      return res.sendStatus(cmn.HTTP_CODE.SUCCESS);
    } catch (err) {
      wutil.log_internal_server_error(req, `Failed to update queries from the database. Got error: ${err}`);
      return wutil.send_internal_server_error(res, 'Failed to update queries from the database');
  ...
  async _submit_query_via_pubsub(req, res, _next) {
    ...
    try {
      ...
      const user_id = wutil.request_to_user_id(req);
      if (pid) {
        const projects = await this.user_service.getUserSavesBy(user_id, {id: pid});
  ...
  async _get_query_status_via_pubsub(req, res, _next) {
    if (!this._is_valid_query_result_request(req)) {
      return wutil.send_error(res, cmn.HTTP_CODE.BAD_REQUEST, 'Malformed Request');
    }
    try {
      const qid = req.params.qid;
      const query_model = await this.query_service.getQueryByPk(qid);
      let status = null;
      if (query_model) {
        status = this.query_service_fe_adapter.queryStatusToFE(query_model);
      } else {
        const status_resp = await this.translator_service.getQueryStatus(qid, this.filters);
  ...
  async _get_user_queries_via_pubsub(req, res, _next) {
    const user_id = wutil.request_to_user_id(req);
    const include_deleted = req.query.include_deleted === 'true';
    try {
      const user_queries = await this.user_service.get_user_queries_map(user_id, include_deleted, this.using_pubsub);
      return res.status(cmn.HTTP_CODE.SUCCESS).json([...user_queries.values()]);
    } catch (err) {
      wutil.log_internal_server_error(req, err);
      wutil.log_internal_server_error(req, `Failed to get queries status for the current user: ${user_id}`);
      return wutil.send_internal_server_error(res, 'Failed to get queries status for the current user');
    }
  }

  async _submit_query_via_polling(req, res, _next) {
    this._log_query_submission_request(req);
    if (!this._is_valid_query_submission_request(req)) {
      return wutil.send_error(res, cmn.HTTP_CODE.BAD_REQUEST, 'Malformed request');
    }
    try {
      ...
      const pk = trapi.get_pk(submit_resp);
      const user_query_model = this.user_service.createUserQuery(user_id, pk, req.body);
      if (!user_query_model) throw new Error(`User service failed to create entry for query ${pk} and user ${user_id}`);
      if (pid) {
        project.data.pks.push(pk);
        const updated_project = await this.user_service.updateUserSave(project);
        if (!updated_project) {
  ...
  async _get_query_status_via_polling(req, res, _next) {
    if (!this._is_valid_query_result_request(req)) {
    ...
    try {
      ...
      const retval = this.translator_service_fe_adapter.queryStatusToFE(status_resp);
  ...
  async _get_user_queries_via_polling(req, res, _next) {
    const user_id = wutil.request_to_user_id(req);
    const include_deleted = req.query.include_deleted === 'true';
    try {
      const user_queries = await this.user_service.get_user_queries_map(user_id, include_deleted, this.using_pubsub);
  ...
  _is_valid_query_submission_request(req) {
    return cmn.is_object(req.body) && wutil.request_to_user_id(req);
  }
  ...
  async _update_user_query(user_id, sid, is_deleted, update) {
    let user_saved_data = await this.user_service.getUserSavesBy(user_id, {id: sid}, is_deleted);
