...
export async function loadChebi() {
  CHEBI_ROLES = await cmn.read_json('./assets/chebi/chebi_roles.json');
  CHEBI_ROOTS = await cmn.read_json('./assets/chebi/chebi_roots.json');
  CHEBI_EXCLUDES = await cmn.read_json('./assets/chebi/chebi_excludes.json');
}
...
export function getHighLevelRole(chebiId) {
  if (cmn.is_missing(CHEBI_ROLES[chebiId])) {
    logger.warn(`Warning: ${chebiId} is an unknwon role`);
    return null;
  }
