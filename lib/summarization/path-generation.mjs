...
function gen_summary_paths_and_edges(analysis_topology, kgraph, start, ends, max_path_len) {
  const analysis_paths = _gen_analysis_paths(analysis_topology, kgraph, start, ends, max_path_len);
  return _gen_summary_paths_and_edges(analysis_paths, analysis_topology, kgraph);
}

function _gen_analysis_paths(analysis_topology, kgraph, start, ends, max_path_len) {
...
function _gen_summary_paths_and_edges(analysis_paths, analysis_topology, kgraph) {
  ...
  const summary_paths = analysis_paths.map(path => {
    ...
    for (let i = 0; i < (path.length - 1); i+=2) {
      ...
      const is_edge_root = edge_topology.is_edge_root(eb);
