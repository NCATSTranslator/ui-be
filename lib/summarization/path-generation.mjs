...
function gen_summary_paths_and_edges(analysis_topology, kgraph, start, ends, max_path_len) {
  const analysis_paths = _gen_analysis_paths(start, ends, analysis_topology, kgraph, max_path_len);
  return _gen_summary_paths_and_edges(analysis_paths, analysis_topology, kgraph);
}

function _gen_analysis_paths(start, ends, analysis_topology, kgraph, max_path_len) {
  return _gen_graph_paths(start, ends);

  function _gen_graph_paths(start, ends) {
    const complete_paths = [];
    const remaining_paths = [[start]];
    while (!cmn.isArrayEmpty(remaining_paths)) {
      const path = remaining_paths.pop();
      const head = path[path.length-1];
      const outedges = analysis_topology.nid_to_outedges(head);
      ...
      if (is_terminal_len) {
        for (let i = 0; i < outedges.length; i++) {
          ...
          if (is_end_node && !is_cycle) {
            path.push(outedge.eid, outedge.target);
        ...
        for (let i = 0; i < outedges.length; i++) {
          ...
          if (!is_cycle) {
            const is_end_node = ends.includes(outedge.target);
            if (is_end_node) {
              complete_paths.push([...path, outedge.eid, outedge.target]);
            }
            remaining_paths.push([...path, outedge.eid, outedge.target]);
          } }
      }
    }
...
function _gen_summary_paths_and_edges(analysis_paths, analysis_topology, kgraph) {
  const edge_topology = analysis_topology.edge_topology;
    ...
    for (let i = 0; i < (path.length - 1); i+=2) {
      ...
      if (!summary_topology.has_edge(eid)) {
        summary_topology.make_edge(eid,
                                   edge_topology.clone_hosts(eb),
                                   edge_topology.clone_support(eb));
      } else {
        summary_topology.add_hosts(eid, edge_topology.clone_hosts(eb));
        summary_topology.add_support(eid, edge_topology.clone_support(eb));
      }
  ...
  for (const eid of summary_topology.edge_ids()) {
    ...
    sedge.isRootPath = edge_metadata.get(eid).is_edge_root;
