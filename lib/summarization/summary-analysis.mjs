...
function summary_analysis_to_summary_paths_and_edges(summary_analysis,
    analysis_paths, kgraph, replace_indirect_edges) {
      ...
      for (const path of paths) {
        ...
        for (let i = 0; i < (path.length-1); i+=2) {
          ...
          if (cmn.is_array_empty(edge_support)) {
            ...
            const is_root = true;
            const eid = id.gen_eid(eb, kgraph, is_edge_inverted, is_root)
            ...
            if (cmn.is_missing(summary_edges[eid])) {
              summary_edge.metadata = {
                edge_bindings: [],
                inverted_id: is_edge_inverted ? id.gen_eid(eb, kgraph, false, is_root) : null,
                is_root: is_root
              }
