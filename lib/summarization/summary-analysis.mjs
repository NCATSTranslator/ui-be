...
function summary_analysis_to_summary_paths_and_edges(summary_analysis,
    analysis_paths, kgraph, replace_indirect_edges, eid_cache = null) {
  if (replace_indirect_edges) {
    return __compress_support(summary_analysis, analysis_paths, kgraph);
  } else {
    return __preserve_support(summary_analysis, analysis_paths, kgraph);
  }

  function __compress_support(summary_analysis, analysis_paths, kgraph) {
    // Dedupe edge_bindings during the recursive walk to avoid duplicate fan-out downstream.
    const eb_sets = {};
    const [compressed_paths, summary_edges] = __compress_graph_support(summary_analysis.root.id);
    const summary_paths = __expand_paths(compressed_paths);
    for (const eid of Object.keys(summary_edges)) {
      if (eb_sets[eid]) {
        summary_edges[eid].metadata.edge_bindings = Array.from(eb_sets[eid]);
      }
    }
    return [
      summary_paths.map(summary_path_ids => {
        const summary_path = new SummaryPath(summary_path_ids);
    ...
    function __compress_graph_support(root_gid) {
      const summary_edges = {};
      const summary_paths = [];
      // Get all paths in the graph
      const paths = analysis_paths.get(root_gid);
      for (const path of paths) {
        const summary_path_ids = [];
        // Look at all node and edge bindings in a path except the last node binding
        for (let i = 0; i < (path.length-1); i+=2) {
          const nb = path[i];
          const eb = path[i+1];
          summary_path_ids.push(id.gen_nid(nb, kgraph));
          const edge_support = summary_analysis.support_graph_ids(eb);
          if (cmn.is_array_empty(edge_support)) {
            // Process edge binding
            // TODO: Metadata handling could be improved
            const is_edge_inverted = pred.is_predicate_inverted(eb, nb, kgraph);
            const is_root = true;
            const eid = id.gen_eid(eb, kgraph, is_edge_inverted, is_root, eid_cache)
            const summary_edge = new SummaryEdge(eid, null, is_root);
            if (!eb_sets[eid]) eb_sets[eid] = new Set();
            eb_sets[eid].add(eb);
            if (cmn.is_missing(summary_edges[eid])) {
              summary_edge.metadata = {
                edge_bindings: [],
                inverted_id: is_edge_inverted ? id.gen_eid(eb, kgraph, false, is_root, eid_cache) : null,
                is_root: is_root
              }
              summary_edges[eid] = summary_edge;
            }
            summary_path_ids.push(eid);
          } else {
            // Replace the edge with all paths of all support graphs
            const support_branches = [];
            for (const gid of edge_support) {
              // Recursive call creates invariant that paths returned will have no edges with support
              // TODO: Every move up the call stack requires copying paths. How to improve?
              const [support_paths, support_edges] = __compress_graph_support(gid);
              if (cmn.is_array_empty(support_paths)) {
                throw new MissingSupportPathsError(gid, eb);
              }
              for (const support_path of support_paths) {
                support_branches.push(support_path.slice(1,-1));
              }
              for (const [eid, support_edge] of Object.entries(support_edges)) {
                if (cmn.is_missing(summary_edges[eid])) {
                  summary_edges[eid] = support_edge;
                } else {
                  summary_edges[eid].merge(support_edge);
                }
              }
            }
            summary_path_ids.push(support_branches);
          }
        }
        // Add the last node binding
        summary_path_ids.push(id.gen_nid(path[path.length-1], kgraph));
        summary_paths.push(summary_path_ids);
      }
      return [summary_paths, summary_edges];
    }

    function __expand_paths(compressed_paths) {
      const summary_paths = [];
      for (const compressed_path of compressed_paths) {
        const expanded_paths = __expand_path(compressed_path);
        for (const expanded_path of expanded_paths) {
          summary_paths.push(expanded_path);
        }
      }
      return summary_paths;

      function __expand_path(compressed_path) {
        let summary_paths = [[]];
        for (const entry of compressed_path) {
          if (!cmn.is_array(entry)) {
            for (const summary_path of summary_paths) {
              summary_path.push(entry);
            }
          } else {
            const new_summary_paths = [];
            const expanded_paths = __expand_paths(entry);
            for (const summary_path of summary_paths) {
              for (const expanded_path of expanded_paths) {
                new_summary_paths.push(summary_path.concat(expanded_path));
              }
            }
            summary_paths = new_summary_paths;
          }
        }
        return summary_paths;
      }
  ...
  function __preserve_support(summary_analysis, analysis_paths, kgraph) {
    const summary_paths = {};
    ...
    for (const [gid, paths] of analysis_paths.entries()) {
      for (const path of paths) {
        ...
        for (let i = 0; i < (path.length -1); i+=2) {
          ...
          const eid = id.gen_eid(eb, kgraph, is_edge_inverted, is_edge_root, eid_cache);
          ...
          if (is_edge_inverted) {
            edge_metadata.get(eid).inverted_id = id.gen_eid(eb, kgraph, false, is_edge_root, eid_cache);
          }
          const nid = id.gen_nid(nb, kgraph);
          summary_path_subgraph.push(nid, eid);
        }
        summary_path_subgraph.push(id.gen_nid(path[path.length-1], kgraph));
    ...
    const summary_edges = {};
    for (const paths of Object.values(summary_paths)) {
      for (const path of paths) {
        path.forEids((eid) => {
          if (summary_edges[eid] === undefined) {
            summary_edges[eid] = new SummaryEdge(eid);
          }
          const summary_edge = summary_edges[eid];
          const metadata = edge_metadata.get(eid);
          metadata.edge_bindings = cmn.distinct_array(metadata.edge_bindings);
          for (const eb of metadata.edge_bindings) {
            for (const gid of summary_analysis.support_graph_ids(eb)) {
              const support_paths = summary_paths[gid];
              if (cmn.is_missing(support_paths) || cmn.is_array_empty(support_paths)) {
                throw new MissingSupportPathsError(gid, eb);
              }
            }
            summary_edge.is_root = metadata.is_root;
...
function _finalize_summary_analysis(summary_analysis, kgraph) {
  ...
  summary_analysis = _trim_summary_analysis_edges(summary_analysis, ((eid) => {
    ...
    return !bl.is_biolink_pred(trapi.get_predicate(kedge));
  }));
...
class SummaryAnalysis {
  ...
  constructor(analysis, kgraph, aux_graphs) {
    ...
    this.score = cmn.json_get(analysis, "score", 0.0);
  ...
  _abinds_to_kgbinds(abinds) {
    return Object.values(abinds).reduce((kgbinds, ab) => {
      return kgbinds.concat(ab.map(map_ab_kgb => {
        return cmn.json_get(map_ab_kgb, 'id');
      }));
    },
    []);
  }
...
class MissingSupportPathsError extends Error {
  constructor(graph_id, edge_binding) {
    super(`Support graph ${graph_id} bound to edge ${edge_binding} produced no paths`);
    this.name = 'MissingSupportPathsError';
  }
}
