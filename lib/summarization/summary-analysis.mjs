      ...
      for (const path of paths) {
        ...
        for (let i = 0; i < (path.length-1); i+=2) {
          ...
          if (cmn.is_array_empty(edge_support)) {
            ...
            const summary_edge = new SummaryEdge(eid, null, null, is_root);
            summary_edge.type = trapi.CONSTANTS.GRAPH.EDGE.TYPE.DIRECT;
        ...
        path.forEids((eid) => {
          ...
          for (const eb of metadata.edge_bindings) {
            for (const gid of summary_analysis.support_graph_ids(eb)) {
              summary_edge.support.push(...(summary_paths[gid].map(summary_path => summary_path.id)));
            }
            summary_edge.is_root = metadata.is_root;
            summary_edge.type = summary_edge.has_support() ? trapi.CONSTANTS.GRAPH.EDGE.TYPE.INDIRECT : trapi.CONSTANTS.GRAPH.EDGE.TYPE.DIRECT;
