...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    Object.keys(pubs).forEach((kl) => {
      ...
      cmn.jsonSet(pubs, kl, klPubs.filter((pub) => {
        ...
        if (ev.is_publication(pub.id)) {
          ...
          const tag_misc = makeTag('p/ev/misc', 'Miscellaneous');
          addTag(edge, tag_misc);
        }
        seenIds.add(pub.id);
    ...
    if (ev.is_ontological(edge.predicate)) {
      ...
      addTag(edge, tag_contological);
    }
    const tag_knowledge_source = makeTag('p/ev/ks', 'Knowledge Source');
    addTag(edge, tag_knowledge_source);
  });
