...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  for (const edge of Object.values(edges)) {
    let found_publication = false;
    let no_evidence = true;
    for (const publications of Object.values(edge.publications)) {
      for (const pub of publications) {
        const [type, _] = ev.idToTypeAndUrl(pub.id);
        found_publication = (type === 'PMID' || type === 'PMC')
        if (found_publication) break;
      }
      if (found_publication) {
        addTag(edge, makeTag('e/ev/publication', 'Publications'));
        no_evidence = false;
        break;
      }
    }
    if (edge.trials.length > 0) {
      addTag(edge, makeTag('e/ev/clinical', 'Clinical Trials'));
      no_evidence = false;
    }
    for (const provenance of edge.provenance) {
      if (provenance.infores === 'infores:ols') {
        addTag(edge, makeTag('e/ev/ontological', 'Ontological'));
        no_evidence = false;
      }
    }
    if (no_evidence) {
      addTag(edge, makeTag('e/ev/other', 'Other'));
    }
  }
