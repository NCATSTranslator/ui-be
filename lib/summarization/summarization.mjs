 ...
 */
export function answers_to_summary(qid, answers, feature_config) {
  ...
  const [sfs, errors] = answersToSmryFgmts(
    answers,
...
function pathAlphaLenCompare(smryPath1, smryPath2) {
...
function pathScoreCompare(smryPath1, smryPath2) {
  if (smryPath1.score >= smryPath2.score) return -1;
  return 1;
}

function queryTypeToPathCompare(queryType) {
  if(trapi.isPathfinderQuery(queryType)) return pathScoreCompare;
  return pathAlphaLenCompare;
}
...
function get_clinical_data() {
  return makeExtractionRule(
    ...
    (obj, key) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const supStudyEntries = attrIter.findAll([bl.tagBiolink('has_supporting_studies')]);
      const extractedTrials = {};
      for (let i = 0; i < supStudyEntries.length; i++) {
        const supStudyEntry = supStudyEntries[i];
        const supStudies = trapi.getAttrVal(supStudyEntry);
        for (let supStudyId of Object.keys(supStudies)) {
          const normalizedId = ev.normalize_prefix(supStudyId);
          if (extractedTrials[normalizedId]) {
            continue;
          }
          const study = supStudies[supStudyId];
          const categories = cmn.jsonGet(study, "category", null);
          if (!cmn.isArray(categories)
              || !categories.includes(bl.tagBiolink("ClinicalTrial"))) {
            continue;
          }
          const extractedTrial = {};
          const phase = cmn.jsonGet(study, "clinical_trial_phase", null);
          extractedTrial.phase = null;
          if (phase !== null) {
            if (phase.endsWith("1_to_2")) {
              extractedTrial.phase = 1.5;
            } else {
              extractedTrial.phase = parseInt(phase[phase.length-1]);
            }
          }
          extractedTrial.status = cmn.jsonGet(study, "clinical_trial_overall_status", null);
          extractedTrial.child = null;
          const age_range = cmn.jsonGet(study, "clinical_trial_age_stage", null);
          if (cmn.isArray(age_range)) {
            extractedTrial.child = age_range.includes("child");
          }
          extractedTrial.size = cmn.jsonGet(study, "clinical_trial_enrollment", null);
          extractedTrial.start_date = cmn.jsonGet(study, "clinical_trial_start_date", null);
          extractedTrial.type = null;
          const enrollment_type = cmn.jsonGet(study, "clinical_trial_enrollment_type", null);
          if (enrollment_type !== null) {
            extractedTrial.type = enrollment_type === "ACTUAL" ? "enrolled" : "anticipated";
          }
          extractedTrial.title = cmn.jsonGet(study, "name", null);
          extractedTrials[normalizedId] = extractedTrial;
        }
      }
...
function getPubs() {
  ...
  return makeExtractionRule(
    ...
    (obj, key, cxt) => {
      ...
      const provenance = bl.inforesToProvenance(cxt.infores);
...
function summarize_kedge(eid, kedge, edgeRules, summary_context) {
  ...
  edge_context.infores = trapi.getPrimarySrc(kedge).infores;
...
function answersToSmryFgmts(answers, knode_summarization_rules, edgeRules, feature_config) {
...
function pidSort(pids, paths, pathCompare) {
  function pidCompare(pid1, pid2) {
    ...
    const comparison = pathCompare(smryPath1, smryPath2);
    if (comparison === 0) {
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, feature_config, errors) {
  function fgmtPathsToResults(fgmtPaths, nodes, queryType) {
    const results = [];
    fgmtPaths.forEach((path) => {
      let rid = path.start;
      if (trapi.isPathfinderQuery(queryType)) {
        rid = id.gen_pid(genMetaPath(path.subgraph, nodes));
      }
  ...
  function extendSmryPaths(paths, newPaths, agents) {
    newPaths.forEach((path) => {
      const smryPath = cmn.jsonGet(paths, path.id, false);
      if (smryPath) {
        smryPath.extendAgents(agents);
        return;
      }
      path.extendAgents(agents);
      cmn.jsonSet(paths, path.id, path);
    });
  }
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors, queryType) {
    ...
    for (const result of results) {
      ...
      expandedResults.push({
        ...
        'subject': start,
        'drug_name': name,
        'paths': pidSort(rootPids, paths, queryTypeToPathCompare(queryType)),
        'object': end,
        'scores': scores[start],
        'tags': tags
      });
  ...
  smryFgmts.forEach((sf) => {
    const agents = sf.agents;
    const newResults = fgmtPathsToResults(sf.paths, nodes, queryType);
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    cmn.jsonUpdate(edge, 'provenance', (provenance) => {
      return provenance.map((p) => {
        const provenanceMapping = cmn.deepCopy(bl.inforesToProvenance(p.infores));
        if (!provenanceMapping) {
          edgeErrorReasons.push(`Found invalid provenance ${p.infores} on edge ${edgeToString(edge)}`);
          return false;
        }
        if (feature_config.pull_source_records && !cmn.isArrayEmpty(p.records)) {
          provenanceMapping.url = p.records[0];
        }
        if (seen.has(p.infores) &&
            seen.get(p.infores).url === provenanceMapping.url) {
          return false;
        }
        seen.set(p.infores, provenanceMapping);
  ...
  try {
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      path.forInternalNids((nid) => {
        const node = nodes[nid];
        mergeTags(path, node, (tag) => { return !isResultTag(tag) && isExternalTag(tag) }); const node_type = node.type;
    ...
    Object.keys(edges).forEach((eid) => {
      const edge = cmn.jsonGet(edges, eid);
      edge.supPaths = pidSort(edge.supPaths, paths, queryTypeToPathCompare(queryType));
    });
