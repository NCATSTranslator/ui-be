...
function summarizeRnode(rnode, kgraph, nodeRules, cxt) {
  const nid = id.genNid(rnode, kgraph);
...
function summarizeRedge(redge, kgraph, edgeRules, cxt, edgeBaseIds) {
  let eid = id.genEid(redge, kgraph);
  if (!edgeBaseIds.has(eid)) {
    eid = id.genEid(redge, kgraph, true);
  }
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      ...
      try {
        ...
        const [normalizedPaths, edgeBases] = ag.finalize_paths(agraph_paths, agraph.edge_topology, kgraph); // C!
        const analysisCxt = {
          agent: agent,
          errors: errors
        };
        ...
        return new SummaryFragment(
          [agent],
          normalizedPaths,
          agraph.node_bindings.map(rnode => { return summarizeRnode(rnode, kgraph, nodeRules, analysisCxt); }),
          new SummaryFragmentEdges(
            edgeBases,
            agraph.edge_bindings.map(redge => {
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  function extractAndFinalizeEdges(edges) {
    function addInverseEdge(edges, edge) {
      const invertedPred = pred.genQualifiedPred(edge, true);
      const sub = edge.subject;
      const obj = edge.object;
      const invertedEid = id.genPid([obj, invertedPred, sub, edge.type]);
      ...
      const unqualifiedInvertedPred = bl.invertBiolinkPred(pred.getMostSpecificPred(edge));
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate', pred.genQualifiedPred(edge));
