...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  const kresult_summarization_rules = _make_extraction_rules(
    [
      _tag_attribute(
        'biothings_annotations',
        (annotations) => {
  ...
  const resultNodeUpdates = [...resultNodes].map((nid) => {
    const knode = trapi.getKnode(nid, kgraph);
    return _make_knode_transform(nid, knode, kresult_summarization_rules, annotation_context);
  });

  _extend_summary_nodes(nodes, nodeUpdates.concat(resultNodeUpdates), ['biothings-annotator']);
