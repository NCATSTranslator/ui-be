...
function _gen_max_phase_tag(node, query_type) {
  ...
  function __is_drug(node, fda_level) {
    return fda_level === 4 || node.type === 'Drug';
  }
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, query_type, errors) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      ...
      trapi_rules.make_rule_map_attributes({
        attr_ids: ['biothings_annotations'],
        target_key: 'curies',
        transform: (annotation) => bta.getCuries(annotation)
      }),
      trapi_rules.make_rule_transform_attribute_value({
        attr_id: 'biothings_annotations',
        target_key: 'species',
        transform: (annotations) => bta.getSpecies(annotations[0])
      }),
      summary_rules.make_rule_tag_attribute({
        attr_id: 'biothings_annotations',
        gen_tag: (annotations) => {
          const tdl = bta.getTdl(annotations[0]);
          if (tdl === null) return null;
          return new taglib.Tag({id: `r/tdl/${tdl.toLowerCase()}`, name: tdl});
        }
      })
    ]
  );
  ...
  const result_summarization_rules = _make_extraction_rules(
    [
      summary_rules.make_rule_tag_attribute({
        attr_id: 'biothings_annotations',
        gen_tag: (annotations) => {
          const fda_approval = bta.getFdaApproval(annotations[0]);
          if (fda_approval === null) {
            return false;
          }
          if (fda_approval < 4) {
            const tags = [];
            ...
            return _CONSTANTS.TAGS.RESULT.FDA.NOT_APPROVED;
          }
          return _CONSTANTS.TAGS.RESULT.FDA.APPROVED;
        }
      }),
      summary_rules.make_rule_tag_attribute({
        attr_id: 'biothings_annotations',
        gen_tag: (annotations, context) => {
          if (trapi.is_gene_chemical_query(context.query_type)) return null;
          const chebi_roles = bta.getChebiRoles(annotations[0]);
          if (chebi_roles === null) return null;
          return chebi_roles.map((role) => {
            return new taglib.Tag({id: `r/role/${role.id}`, name: cmn.titleize(role.name)})
          });
        }
      }),
      trapi_rules.make_rule_transform_attribute_value({
        attr_id: 'biothings_annotations',
        target_key: 'indications',
        transform: (annotations) => bta.getDrugIndications(annotations[0])
      }),
      summary_rules.make_rule_tag_attribute({
        attr_id: 'biothings_annotations',
        gen_tag: (annotations) => {
          ...
          if (otc === null) return [];
          return [new taglib.Tag({id: `r/otc/${otc.id}`, name: otc.name})];
        }
      })
    ]
  );
      ...
      if (path_start.indications !== undefined) {
        ...
        for (let i = 0; i < path_end_mesh_ids.length; i++) {
          if (path_start_indications.has(path_end_mesh_ids[i])) {
            is_path_indicated = taglig.has_tag(path_start, _CONSTANTS.TAGS.RESULT.FDA.APPROVED);
            break;
          }
        }
...
const _CONSTANTS = Object.freeze({
  TAGS: {
    PATH: {
      ...
      TYPE: {
        DIRECT: new taglib.Tag('p/pt/lkup', 'Direct'),
        INDIRECT: new taglib.Tag('p/pt/inf', 'Indirect')
      }
    },
    RESULT: {
      CHEMICAL_CLASS {
