...
import * as sn from './SummaryNode.mjs';
...
function answers_to_summary(qid, answers, max_hops) {
  if (!cmn.isArray(answers) || cmn.isArrayEmpty(answers)) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      _aggregate_property('name', ['names']),
      _aggregate_property('categories', ['types']),
      _aggregate_and_transform_attributes([bl.tagBiolink('xref')], 'curies', (xref) => {
        return bl.isValidCurie(xref) ? xref : [];
      }),
      _aggregate_attributes([bl.tagBiolink('description')], 'descriptions'),
    ]);

  const kedge_summarization_rules = _make_extraction_rules(
    [
      _transform_property_rule(
        'predicate',
        (obj, key) => bl.sanitizeBiolinkItem(cmn.jsonGet(obj, key))),
      _aggregate_and_transform_property(
        trapi.CONSTANTS.GRAPH.SOURCES.KEY,
        ['provenance'],
        (obj, key) => trapi.get_primary_source(obj)),
      _transform_property_rule('qualifiers', (obj, key) => cmn.jsonGet(obj, key, false)),
      _get_property_rule('subject'),
      _get_property_rule('object'),
      _get_publications(),
      _get_publication_supporting_text(),
      _get_publication_semmed_sentences(),
      _get_clinical_trial_metadata()
    ]);
  ...
  const summary = _summary_fragments_to_summary(
      qid,
      summary_fragments,
      _get_answers_kgraph(answers),
      query_type,
      errors);
  return summary;
}
 ...
 * @returns {function} - The extraction rule.
 */
function _transform_property_rule(key, transform) {
 ...
 * @returns {function} - The extraction rule.
 */
function _get_property_rule(key) {
  return _transform_property_rule(key, (obj, key) => cmn.jsonGet(obj, key));
}
 ...
 * @returns {function} - The extraction rule.
 */
function _aggregate_and_transform_property(key, kpath, transform) {
  return makeExtractionRule(
    key,
    transform,
    (property, acc) => {
 ...
 * @returns {function} - The extraction rule.
 */
function _aggregate_property(key, kpath) {
  return _aggregate_and_transform_property(key, kpath, (obj) => { return cmn.jsonGet(obj, key); });
}
 ...
 * @returns {function} - The extraction rule.
 */
function _rename_and_transform_attribute(attrId, kpath, transform) {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const attrVal = attrIter.find_one_val([attrId]);
 ...
 * @returns {function} - The extraction rule.
 */
function _aggregate_and_transform_attributes(attrIds, accKey, transform) {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key, cxt) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const result = attrIter.find_all_val(attrIds);
 ...
 * @returns {function} - The extraction rule.
 */
function _aggregate_attributes(attrIds, accKey) {
  return _aggregate_and_transform_attributes(
    attrIds,
    accKey,
    cmn.identity)
}
 ...
 * @returns {function} - The extraction rule.
 */
function _tag_attribute(attrId, transform) {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key, cxt) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const result = attrIter.find_one_val([attrId]);
 ...
 * @returns {function} - The extraction rule.
 */
function _get_publication_supporting_text() {
  ...
  return makeExtractionRule(
    ...
    (obj, key) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const supTextEntries = attrIter.find_all([supStudyId]);
      const supText = {};
      supTextEntries.forEach(supTextEntry => {
        const entryAttrs = trapi.get_attrs(supTextEntry);
        const supTextData = {};
        entryAttrs.forEach(attr => {
          const aid = trapi.get_attr_id(attr);
          const av = trapi.get_attr_val(attr);
          switch (aid) {
            case pubsId:
    ...
    (supText, acc) =>
    {
      const currentSupText = cmn.jsonSetDefaultAndGet(acc, _CONSTANTS.SUPPORTING_TEXT, {});
 ...
 * @returns {function} - The extraction rule.
 */
function _get_publication_semmed_sentences() {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key) => {
      const attr_itr = new trapi.AttributeIterator(obj[key]);
      const publication_data = attr_itr.find_all_val(['bts:sentence']);
    ...
    (supporting_text, acc) => {
      const current_support_text = cmn.jsonSetDefaultAndGet(acc, _CONSTANTS.SUPPORTING_TEXT, {});
...
function _get_clinical_trial_metadata() {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const supTrials = attrIter.find_all([bl.tagBiolink('supporting_study')]);
      const extractedTrials = {};
      for (let i = 0; i < supTrials.length; i++) {
        const supTrial = supTrials[i];
        const supTrialId = trapi.get_attr_val(supTrial);
        if (extractedTrials[supTrialId]) {
          continue;
        }
        extractedTrials[supTrialId] = {};
        const extractedTrial = extractedTrials[supTrialId];
        const supTrialAttrs = trapi.get_attrs(supTrial);
        for (let j = 0; j < supTrialAttrs.length; j++) {
          const attr = supTrialAttrs[j];
          switch(trapi.get_attr_id(attr)) {
            case 'clinical_trial_phase':
              extractedTrial.phase = trapi.get_attr_val(attr); break;
            case 'clinical_trial_status':
              extractedTrial.status = trapi.get_attr_val(attr); break;
            case 'child':
              extractedTrial.child = trapi.get_attr_val(attr); break;
            case 'start_date':
              extractedTrial.start_date = trapi.get_attr_val(attr); break;
            case 'study_size':
              extractedTrial.size = trapi.get_attr_val(attr); break;
            case 'enrollment_type':
              const type = trapi.get_attr_val(attr);
              extractedTrial.type = type === 'ACTUAL' ? 'enrolled' : 'anticipated';
              break;
            case 'brief_title':
              const title = trapi.get_attr_val(attr);
    ...
    (supTrials, acc) => {
      const curSupTrials = cmn.jsonSetDefaultAndGet(acc, _CONSTANTS.SUPPORTING_TRIALS, {});
...
function _get_publications() {
  ...
  return makeExtractionRule(
    ...
    (pubs, acc) => {
      const currentPubs = cmn.jsonSetDefaultAndGet(acc, _CONSTANTS.PUBLICATIONS, {});
...
function getResultStartAndEnd(result, startKey, endKey) {
  ...
  function flattenBinding(result, key) {
    const nodeBinding = trapi.get_node_binding(result, key);
...
function genMaxPhaseTag(node, queryType) {
  ...
  if (!trapi.is_valid_query(queryType) || trapi.is_gene_chemical_query(queryType)) {
    return false;
  }
...
function _make_kedge_transform(eid, kedge, kedge_summarization_rules, summary_context) {
  ...
  edge_context.primarySrc = trapi.get_primary_source(kedge);
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, queryType, errors) {
  function fgmtPathsToResultsAndPaths(fgmtPaths, nodes, queryType) {
    ...
    fgmtPaths.forEach((path) => {
      ...
      let rid = path[0];
      if (trapi.is_pathfinder_query(queryType)) {
  ...
  function _extend_summary_nodes(nodes, nodeUpdates, agents) {
    _extend_summary_graph_elements(nodes, nodeUpdates, agents, () => new sn.SummaryNode());
  }
  ...
  function extendSmryTrials(smryTrials, edge) {
    const trials = cmn.jsonGet(edge, _CONSTANTS.SUPPORTING_TRIALS, {});
  ...
  function extractAndFinalizeEdges(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      const supText = cmn.jsonGet(edge, _CONSTANTS.SUPPORTING_TEXT, {});
      Object.keys(edgePubs).forEach((kl) => {
        edgePubs[kl] = edgePubs[kl].map((pub) => {
          return { id: pub.id, support: supText[pub.id] || null };
        });
      });
      delete edge[_CONSTANTS.SUPPORTING_TEXT];
      edge.trials = Object.keys(cmn.jsonGet(edge, _CONSTANTS.SUPPORTING_TRIALS, []));
      delete edge[_CONSTANTS.SUPPORTING_TRIALS];
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    Object.keys(pubs).forEach((kl) => {
      ...
      cmn.jsonSet(pubs, kl, klPubs.filter((pub) => {
        ...
        if (ev.is_publication(pub.id)) {
          const tag_pubs = makeTag('p/ev/publications', 'Publications');
          addTag(edge, tag_pubs);
        }
        seenIds.add(pub.id);
    ...
    if (!cmn.isArrayEmpty(Object.keys(edge.supporting_trials))) {
      const tag_clinical = makeTag('p/ev/clinical', 'Clinical Trials');
      addTag(edge, tag_clinical);
    }
    // TODO: Refine meaning of an ontological edge
    if (ev.is_ontological(edge.predicate)) {
      const tag_ontological = makeTag('p/ev/ontology', 'Accepted Ontology');
      addTag(edge, tag_ontological);
    }
    const tag_other = makeTag('p/ev/other', 'Other');
    addTag(edge, tag_other);
  });
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      _rename_and_transform_attribute(
      ...
      _aggregate_and_transform_attributes(
        ['biothings_annotations'],
        'curies',
  ...
  const result_summarization_rules = _make_extraction_rules(
    [
      _tag_attribute(
        'biothings_annotations',
        (annotations) => {
      ...
      _tag_attribute(
        'biothings_annotations',
        (annotations, cxt) => {
          if (trapi.is_gene_chemical_query(cxt.queryType)) return [];
  ...
  const result_nodes = new Set();
  results.forEach((result) => {
    const pids = cmn.jsonGet(result, 'paths');
    pids.forEach((pid) => {
      const summary_path = _pid_to_summary_path(pid, paths);
      result_nodes.add(summary_path.start);
    });
  });
  ...
  const node_transforms = Object.keys(nodes).map((nid) => {
    const knode = trapi.get_knode(nid, kgraph);
    return _make_knode_transform(nid, knode, knode_summarization_rules, annotation_context);
  });

  const result_node_transforms = [...result_nodes].map((nid) => {
    const knode = trapi.get_knode(nid, kgraph);
    return _make_knode_transform(nid, knode, result_summarization_rules, annotation_context);
  });

  _extend_summary_nodes(nodes, node_transforms.concat(result_node_transforms), ['biothings-annotator']);
  extendSmryErrors(errors, annotation_context.errors);
  ...
  Object.keys(paths).forEach((pid) => {
    ...
    if (trapi.is_chemical_disease_query(queryType)) {
      ...
      if (path_start.indications !== undefined) {
        ...
        for (let i = 0; i < path_end_mesh_ids.length; i++) {
          if (path_start_indications.has(path_end_mesh_ids[i])) {
            is_path_indicated = path_start.tags['r/fda/4'] !== undefined;
            break;
          }
        }
        ...
        addTag(path_start, tag_indicated);
      }

      cmn.jsonDelete(path_start, 'indications');
    }
    ...
    path.forInternalNids((nid) => {
      const node = nodes[nid];
      mergeTags(path, node, (tag) => { return !isResultTag(tag) && isExternalTag(tag) }); const node_type = node.get_specific_type();
