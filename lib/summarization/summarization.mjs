  ...
  try {
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      path.forEids((eid) => {
        ...
        // Generate tags for path evidence
        if (!cmn.isArrayEmpty(edge.trials)) {
          const tag_trial = makeTag('p/ev/clinical', 'Clinical Trials');
          addTag(path, tag_trial);
        }
        for (let pubs of Object.values(edge.publications)) {
          for (let pub of pubs) {
            if (ev.is_publication(pub.id)) {
              const tag_publication = makeTag('p/ev/publication', 'Publications');
              addTag(path, tag_publication);
            } else {
              const tag_misc = makeTag('p/ev/misc', 'Miscellaneous');
              addTag(path, tag_misc);
            }
          }
        }
        // This is a rough estimate of what an ontological edge is to start with and can be refined later
        if (ev.is_ontological(edge.predicate)) {
          const tag_ontology = makeTag('p/ev/ontological', 'Accepted Ontology');
          addTag(path, tag_ontology);
        }
      });
      const tag_knowledge_source = makeTag('p/ev/ks', 'Knowledge Source');
      addTag(path, tag_knowledge_source);
