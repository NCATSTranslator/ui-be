...
function answers_to_summary(qid, answers, max_hops) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      trapi.make_rule_aggregate_property({
        source_key: 'name',
        target_kpath: ['names']
      }),
      trapi.make_rule_aggregate_property({
      ...
      trapi.make_rule_map_attributes({
        attr_ids: [bl.tagBiolink('xref')],
        target_key: 'curies',
        transform: xref => bl.isValidCurie(xref) ? xref : []
      }),
      trapi.make_rule_collect_attributes({
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      trapi.make_rule_transform_and_rename_attribute( //TODO: replace with biothings rule
