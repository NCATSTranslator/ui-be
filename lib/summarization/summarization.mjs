...
import * as ag from './analysis_graph.mjs';
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      ...
      try {
        ...
        const [normalizedPaths, edgeBases] = finalizePaths(agraph_paths, agraph.edge_topology, kgraph); // C!
        const analysisCxt = {
          agent: agent,
          errors: errors
        };
        ...
        console.error(err);
        if (err instanceof ag.EdgeBindingNotFoundError) {
