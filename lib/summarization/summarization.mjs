...
import * as ag from './analysis-graph.mjs';
import * as pgen from './path-generation.mjs';
...
function summarizeRnode(nid, knode, nodeRules, summary_context) {
  return cmn.make_pair('id',         nid,
                       'transforms', nodeRules(knode, summary_context));
}

function summarizeRedge(eid, kedge, edgeRules, summary_context) {
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      ...
      try {
        ...
        const agraph = ag.analysis_to_agraph(analysis, kgraph, auxGraphs);
        const agraph_paths = pgen.gen_paths(agraph, kgraph, start, ends, max_path_len);
        ...
        const [summary_paths, edges] = pgen.finalize_paths(agraph_paths, agraph.edge_topology, kgraph);
        const analysis_context = {
          agent: agent,
          errors: errors
        };

        const node_updates = agraph.node_bindings.map(nb => {
          const knode = trapi.getKnode(nb, kgraph);
          return summarizeRnode(nb, knode, nodeRules, analysis_context);
        });
        const edge_updates = [];
        for (const [eid, edge] of Object.entries(edges)) {
          for (const eb of edge.metadata.edge_bindings) {
            ...
            edge_updates.push(summarizeRedge(eid, kedge, edgeRules, analysis_context));
          }
        }
        return new SummaryFragment(
          ...
          summary_paths,
          node_updates,
          new SummaryFragmentEdges(edges, edge_updates)
        );
      } catch (err) {
        console.error(err);
        ...
        return makeErrSmryFgmt(agent, 'Unknown error while building AGraph');
      }
    }
    ...
    try {
      ...
      if (!resultSmryFgmt.isEmpty()) {
        ...
        const rid = id.genNid(rnodeStart, kgraph); // The first node uniquely identifies a result.
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  function fgmtPathsToResultsAndPaths(fgmtPaths, nodes, queryType) {
    ...
    fgmtPaths.forEach((path) => {
      const pid = id.genPid(path);
      let rid = path[0];
      if (trapi.isPathfinderQuery(queryType)) {
        rid = id.genPid(genMetaPath(path, nodes));
      }
  ...
  function extractAndFinalizeEdges(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate_url', bl.predToUrl(pred.getMostSpecificPred(edge)));
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors, queryType) {
    ...
    function genId(nodes, smryPath, queryType) {
      if (trapi.isPathfinderQuery(queryType)) {
        const metaPath = genMetaPath(smryPath.subgraph, nodes);
        return id.genPid(metaPath);
      }
      return id.genPid([smryPath.start, smryPath.end]);
    }
  ...
  try {
    ...
    const nodeUpdates = Object.keys(nodes).map((nid) => {
      ...
      return summarizeRnode(nid, knode, nodeRules, annotationCxt);
    });

    const resultNodeUpdates = [...resultNodes].map((nid) => {
      const knode = trapi.getKnode(nid, kgraph);
      return summarizeRnode(nid, knode, resultNodeRules, annotationCxt);
    });
