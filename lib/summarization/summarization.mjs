...
import * as anys from './analysis.mjs';
...
function answersToSmryFgmts(answers, nodeRules, edgeRules, max_hops) {
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      ...
      try {
        const rgraph = anys.analysis_to_agraph(analysis, kgraph, auxGraphs); // R!
        ...
        const rgraph_paths = gen_paths(start, ends, max_path_len, map_rnode_outedges);
        const [normalizedPaths, edgeBases] = finalizePaths(rgraph_paths, rgraph.edge_topology, kgraph); // C!
        const analysisCxt = {
          agent: agent,
          errors: errors
        };
        ...
        console.error(err);
        if (err instanceof anys.EdgeBindingNotFoundError) {
