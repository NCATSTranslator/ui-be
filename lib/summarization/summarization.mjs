...
import * as atop from './analysis-topology.mjs';
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysis_to_fragment(analysis, kgraph, auxiliary_graphs, start, ends) {
      ...
      try {
        ...
        const analysis_topology = atop.analysis_to_analysis_topology(analysis, kgraph, auxiliary_graphs);
        const [summary_paths, summary_edges] = pgen.gen_summary_paths_and_edges(analysis_topology,
            kgraph, start, ends, max_path_len);
        const analysis_context = {
          agent: agent,
          errors: errors
        };
        const knode_properties = analysis_topology.node_ids.map(nb => {
