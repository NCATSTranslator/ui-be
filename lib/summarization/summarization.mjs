  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysis_to_fragment(analysis, kgraph, auxiliary_graphs, start, ends) {
      ...
      try {
        ...
        const analysis_graph = ag.analysis_to_analysis_graph(analysis, kgraph, auxiliary_graphs);
        const [summary_paths, summary_edges] = pgen.gen_summary_paths_and_edges(analysis_graph,
            kgraph, start, ends, max_path_len);
        const analysis_context = {
          agent: agent,
          errors: errors
        };
        const knode_properties = analysis_graph.node_bindings.map(nb => {
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  try {
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      // Generate tags for knowledge sources
      smryPath.forEids((eid) => {
        const edge = edges[eid];
        for (const prov of edge.provenance) {
          const provTag = makeTag(`p/ks/${prov.name}`, prov.name);
