...
class SummaryEdge {
  ...
  static from_object(obj) {
    ...
    const edge = new SummaryEdge(obj.id, obj.aras, obj.is_root);
    ...
    edge.source_time = obj.source_time ?? null;
    taglib.tags_from_raw_obj(edge, obj.tags);
    return edge;
  }

  constructor(id, aras, is_root) {
    this.id = id;
    this.aras = aras || [];
  ...
  merge(summary_edge) {
    assert.ok(this.id === summary_edge.id);
