...
const TDL_LABELS = Object.freeze({
  'tclin': 'Tclin (approved drug)',
  'tchem': 'Tchem (reported tool compound)',
  'tbio':  'Tbio (known molecular mechanism)',
  'tdark': 'Tdark (understudied)'
});
...
function make_tdl_tag(tdl) {
  const tdl_tag = tdl.toLowerCase();
  const tdl_label = TDL_LABELS[tdl_tag];
  return new taglib.Tag({id: `${TAG_SCOPE.NODE}/tdl/${tdl_tag}`, name: tdl_label});
}
...
function decorate_indication_tag(path, nodes) {
  ...
  if (!cmn.is_missing(indications)) {
    const indications_set = new Set(indications.flatMap(indication => indication.ids));
    const path_end = nodes[path.end];
    for (const curie of path_end.curies) {
      if (indications_set.has(curie)) {
