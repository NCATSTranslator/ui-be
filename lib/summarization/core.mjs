 ...
 */
function answers_to_summary(qid, answers, feature_config) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      ...
      trapi_rules.make_rule_transform_attribute_value({
        attr_id: bl.tag_biolink('synonym'),
        transform: cmn.identity,
        update: (target, synonyms) => {
          const current = cmn.json_set_default_and_get(target, "synonyms", []);
      ...
      trapi_rules.make_rule_map_attributes({
        attr_ids: [bl.tag_biolink('same_as')],
        target_key: 'curies',
        transform: attr => {
          const valid_xrefs = [];
          trapi.get_attr_val(attr).forEach(xref => {
            if (bl.is_valid_curie(xref)) {
      ...
      trapi_rules.make_rule_collect_attribute_values({
        attr_ids: [bl.tag_biolink('description')],
  ...
  const kedge_summarization_rules = _make_extraction_rules(
    [
      trapi_rules.make_rule_transform_property({
        ...
        transform: (obj, key) => bl.sanitize_biolink_item(cmn.json_get(obj, key))
      }),
      ...
      trapi_rules.make_rule_transform_property({
        ...
        transform: (obj, key) => cmn.json_get(obj, key, false)
      }),
  ...
  const [summary_fragments, errors] = _answers_to_summary_fragments(answers,
      knode_summarization_rules, kedge_summarization_rules, feature_config);
  const summary = _summary_fragments_to_summary(qid, summary_fragments,
      _get_answers_kgraph(answers), query_type, feature_config, errors);
  return summary;
}

class Summary {
  constructor(meta, results, paths, nodes, edges, pubs, trials, provenance, tags, errors) {
    this.meta = meta || {};
    ...
    Object.keys(this.publications).forEach(pid => {
      this.publications[pid] = this.publications[pid].to_obj();
    });
  }
...
class SummaryFragment {
  ...
  pushScore(rid, scoringComponents) {
    const resultScores = cmn.json_set_default_and_get(this.scores, rid, []);
  ...
  pushError(agent, error) {
    const currentError = cmn.json_set_default_and_get(this.errors, agent, []);
  ...
  _mergeFgmtObjects(obj1Prop, obj2Prop) {
    Object.keys(obj2Prop).forEach((k) => {
      const current = cmn.json_set_default_and_get(obj1Prop, k, []);
...
class SummaryFragmentEdges {
  ...
  isEmpty() {
    return cmn.is_object_empty(this.base) && cmn.is_array_empty(this.updates);
  }

  merge(smryFgmtEdges) {
    Object.keys(smryFgmtEdges.base).forEach((eid) => {
      const currentEdge = cmn.json_set_default_and_get(this.base, eid, new SummaryEdge(eid));
...
function _path_alpha_len_compare(summary_path_a, summary_path_b) {
...
function _path_score_compare(summary_path_a, summary_path_b) {
  if (summary_path_a.score >= summary_path_b.score) return -1;
  return 1;
}

function _query_type_to_path_compare(query_type) {
  if (trapi.is_pathfinder_query(query_type)) return _path_score_compare;
  return _path_alpha_len_compare;
}
...
class SummaryPublication {
  constructor(type, url) {
    this.type = type;
    this.url = url;
    this.infores = new Set();
  }

  register_infores(infores) {
    this.infores.add(infores);
  }
...
function _get_result_endpoints(result, start_key, end_key) {
  ...
  function __flatten_binding(result, key) {
    ...
    return node_bindings.map((entry) => {
      const endpoint = cmn.json_get(entry, 'id');
      return endpoint;
    });
  }
}
...
const _TAG_SCOPE = Object.freeze({
  EDGE: 'e',
  NODE: 'n',
  PATH: 'p',
  RESULT: 'r'
...
function _is_result_tag(tag) {
  return tag.id.startsWith(`${_TAG_SCOPE.RESULT}/`);
}

function _promote_tag(tag, scope) {
...
function _make_kedge_transform(eid, kedge, kedge_summarization_rules, summary_context) {
  const edge_context = cmn.deep_copy(summary_context)
  const source = trapi.get_primary_source(kedge);
  const provenance = bl.infores_to_provenance(source.infores);
  if (cmn.is_missing(provenance)) throw new _MissingInforesError(source.infores);
  edge_context.infores = source.infores;
...
function updateErrorsFromEdge(edge, errors, edgeErrorReasons) {
  ...
  if (edgeAras.length !== 1) {
    edgeErrors = cmn.json_set_default_and_get(errors, 'unknown', []);
  } else {
    edgeErrors = cmn.json_set_default_and_get(errors, edgeAras[0], []);
  }
...
function _answers_to_summary_fragments(
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysis_to_fragment(analysis, kgraph, auxiliary_graphs, start, ends) {
      const agent = cmn.json_get(analysis, 'resource_id', false);
      if (!agent) {
        return make_error_fragment('unknown', 'Expected analysis to have resource_id');
      }

      try {
        const max_path_len = (2 * feature_config.max_hops) + 1;
        const summary_analysis = sa.analysis_to_summary_analysis(analysis, kgraph, auxiliary_graphs);
        if (!summary_analysis) {
          //TODO:[error] could return a more useful error message instead of just false
          return make_error_fragment(agent, 'Summary analysis construction failed');
        }
        ...
        const [summary_paths, summary_edges] = sa.summary_analysis_to_summary_paths_and_edges(
          summary_analysis, analysis_paths, kgraph, feature_config.replace_indirect_edges);
...
function _get_summary_path(pid, paths) {
  return cmn.json_get(paths, pid);
}

function _pid_sort(pids, paths, path_compare) {
  ...
  function __pid_compare(pid_a, pid_b) {
    ...
    const comparison = path_compare(summary_path_a, summary_path_b);
    if (comparison === 0) {
...
function isRootPath(pid, paths, edges) {
  ...
  summary_path.forEids((eid) => {
    isRoot = isRoot && edges[eid].is_root;
  });
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, query_type, feature_config, errors) {
  function fgmtPathsToResults(fgmtPaths, nodes, query_type) {
    const results = [];
    fgmtPaths.forEach((path) => {
      let rid = path.start;
      if (trapi.is_pathfinder_query(query_type)) {
        rid = id.gen_pid(_gen_metapath(path.subgraph, nodes));
      }
      results.push(cmn.make_pair('start', rid, 'pid', path.id));
    });
  ...
  function extendSmryResults(results, newResults) {
    newResults.forEach((result) => {
      let existingResult = cmn.json_set_default_and_get(results, result.start, {});
      let paths = cmn.json_set_default_and_get(existingResult, 'paths', [])
  ...
  function extendSmryPaths(paths, newPaths, agents) {
    newPaths.forEach((path) => {
      const summary_path = cmn.json_get(paths, path.id, false);
      if (summary_path) {
        summary_path.extendAgents(agents);
        return;
      }
      path.extendAgents(agents);
      cmn.json_set(paths, path.id, path);
    });
  }

  function _extend_summary_graph_elements(objs, updates, agents, defaultValue) {
    updates.forEach((update) => {
      const obj = cmn.json_set_default_and_get(objs, update.id, defaultValue(update.id));
  ...
  function extendSmryEdges(edges, edgeFgmts, agents) {
    Object.keys(edgeFgmts.base).forEach((eid) => {
      const edge = cmn.json_set_default_and_get(edges, eid, new SummaryEdge(eid));
      edge.merge(edgeFgmts.base[eid]);
    });

    _extend_summary_graph_elements(edges, edgeFgmts.updates, agents, (eid) => new SummaryEdge(eid));
  }

  function extendSmryScores(scores, newScores) {
    Object.keys(newScores).forEach((rid) => {
      const currentScores = cmn.json_set_default_and_get(scores, rid, []);
  ...
  function extendSmryErrors(errors, newErrors) {
    Object.keys(newErrors).forEach((agtId) => {
      const currentErrors = cmn.json_set_default_and_get(errors, agtId, []);
      currentErrors.push(...newErrors[agtId]);
    });
  }

  function extendSmryPubs(smryPubs, edge) {
    const pubs = cmn.json_get(edge, 'publications', {});
    Object.keys(pubs).forEach((ks) => {
      const pubData = cmn.json_get(pubs, ks, []);
      pubData.forEach((pub) => {
        const pid = pub.id;
        let publication = cmn.json_get(smryPubs, pid, null);
        if (null === publication) {
          const [type, url] = ev.id_to_type_and_url(pid);
          publication = new SummaryPublication(type, url);
          cmn.json_set(smryPubs, pid, publication);
        }
        publication.register_infores(pub.infores);
      });
    });
  }

  function extendSmryTrials(smryTrials, edge) {
    const trials = cmn.json_get(edge, trapi.CONSTANTS.ATTRIBUTES.ID.SUPPORTING_TRIALS, {});
    Object.keys(trials).forEach((ctid) => {
      const trial = trials[ctid];
      trial.id = ctid;
      const [_, url] = ev.id_to_type_and_url(ctid);
      trial.url = url;
      smryTrials[ctid] = trial;
    });
  }

  function extendSmryProvenance(summaryProvenance, edge) {
    edge.provenance.forEach(source => {
      const infores = source.infores;
      const provenance = bl.infores_to_provenance(infores);
      if (!provenance || !cmn.is_missing(summaryProvenance[infores])) return;
      summaryProvenance[infores] = provenance;
    });
  }

  function filter_redundant_provenance(edge) {
    const infores_with_records = new Set();
    for (let i = 0; i < edge.provenance.length; i++) {
      const source = edge.provenance[i];
      if (source.records && source.records.length > 0) {
        infores_with_records.add(source.infores);
      }
    }
    // Also prioritize source records over default URLs
    edge.provenance = edge.provenance.filter(source => {
      const has_records = source.records && source.records.length > 0;
      return has_records || !infores_with_records.has(source.infores);
    });
  }
  ...
  function extractAndFinalizeEdges(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      extendSmryProvenance(provenance, edge);
      edge.knowledge_level = provenance[edge.provenance[0].infores].knowledge_level;
      filter_redundant_provenance(edge);
      const edgePubs = cmn.json_get(edge, 'publications', {})
      const supText = cmn.json_get(edge, trapi.CONSTANTS.ATTRIBUTES.ID.SUPPORTING_TEXT, {});
      Object.keys(edgePubs).forEach((kl) => {
        edgePubs[kl] = edgePubs[kl].map((pub) => {
          return {
            id: pub.id,
            support: supText[pub.id] ?? null,
            infores: pub.infores
          };
        });
      });
      delete edge[trapi.CONSTANTS.ATTRIBUTES.ID.SUPPORTING_TEXT];
      edge.trials = Object.keys(cmn.json_get(edge, trapi.CONSTANTS.ATTRIBUTES.ID.SUPPORTING_TRIALS, []));
      delete edge[trapi.CONSTANTS.ATTRIBUTES.ID.SUPPORTING_TRIALS];
      const unqualified_predicate = pred.get_most_specific_predicate(edge);
      const canonical_predicate = pred.gen_canonical_qualified_predicate(edge);
      taglib.set_tag(edge, new taglib.Tag({
        id: `${_TAG_SCOPE.EDGE}/pred/${canonical_predicate}`,
        name: canonical_predicate
      }));
      cmn.json_set(edge, 'predicate', pred.gen_qualified_predicate(edge, edge.is_inverted()));
      cmn.json_set(edge, 'predicate_url', bl.pred_to_url(unqualified_predicate));
      cmn.json_set(edge, 'description', bl.get_predicate_description(unqualified_predicate));
      if (edge.is_inverted()) {
    ...
    for (const result of results) {
      ...
      if (cmn.is_array_empty(root_pids)) {
        ...
        for (const ara of aras) {
          const ara_errors = cmn.json_set_default_and_get(errors, ara, []);
      ...
      const result_id = __gen_id(nodes, summary_path, query_type);
      const expanded_result = expanded_results_map[result_id];
      if (cmn.is_missing(expanded_result)) {
        expanded_results_map[result_id] = {
          id: result_id,
          subject: start,
          drug_name: name,
          paths: root_pids,
        ...
        expanded_result.paths.push(...root_pids);
        Object.assign(expanded_result.tags, tags);
      }
    }
    const expanded_results = Object.values(expanded_results_map);
    for (let i = 0; i < expanded_results.length; i++) {
      const result = expanded_results[i];
      _pid_sort(result.paths, paths, _query_type_to_path_compare(query_type));
    }
  ...
  Object.values(nodes).forEach(node => {
    node.types.sort(bl.biolink_class_cmp_fn);
  });

  smryFgmts.forEach((sf) => {
    const agents = sf.agents;
    const newResults = fgmtPathsToResults(sf.paths, nodes, query_type);
    extendSmryResults(results, newResults);
    extendSmryPaths(paths, sf.paths, agents);
  });

  results = Object.values(results).map(cmn.obj_remove_duplicates)
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    cmn.obj_remove_duplicates(edge);

    // Remove duplicates from publications and generate pub/misc tags
    const pubs = cmn.json_get(edge, 'publications', {});
    Object.keys(pubs).forEach((kl) => {
      const klPubs = cmn.json_get(pubs, kl, []);
      const seenIds = new Set();
      cmn.json_set(pubs, kl, klPubs.filter((pub) => {
        const shouldInclude = !seenIds.has(pub.id);
        if (ev.is_publication(pub.id)) {
          taglib.set_tag(edge, _CONSTANTS.TAGS.EDGE.EVIDENCE.PUBLICATIONS)
        }
        seenIds.add(pub.id);
    ...
    edge.provenance.forEach(source => {
      if (!bl.is_valid_infores(source.infores)) {
        edgeErrorReasons.push(`Found invalid infores: ${source.infores} on edge ${edgeToString(edge)}`);
      }
    });
    ...
    if (!cmn.is_array_empty(Object.keys(edge.supporting_trials))) {
      taglib.set_tag(edge, _CONSTANTS.TAGS.EDGE.EVIDENCE.CLINICAL);
    }
    // TODO: Refine meaning of an ontological edge
    if (ev.is_ontological(edge.predicate)) {
      taglib.set_tag(edge, _CONSTANTS.TAGS.EDGE.EVIDENCE.ONTOLOGICAL);
    }
    taglib.set_tag(edge, _CONSTANTS.TAGS.EDGE.EVIDENCE.OTHER);
  });

  [edges, pubs, trials, summaryProvenance] = extractAndFinalizeEdges(edges);

  const meta = new SummaryMetadata(qid, cmn.distinct_array(smryFgmts.map((sf) => {
    return sf.agents;
  }).flat()));
  ...
  Object.keys(nodes).forEach((nid) => {
    ...
    cmn.obj_remove_duplicates(node);
    node.types.sort(bl.biolink_class_cmp_fn);
    _conflate_gene_and_protein(node);

    // Provide a CURIE as a default value if the node has no name
    const node_names = node.names;
    pushIfEmpty(node_names, nid);

    const provenance = bl.curie_to_normalized_url(nid, node.curies);
    if (!provenance) {
    ...
    if (bta.is_gene(node)) {
      const tdls = node.annotations.gene.tdl;
      for (let i = 0; i < tdls.length; i++) {
        const tdl = tdls[i];
        taglib.set_tag(node, new taglib.Tag({id: `${_TAG_SCOPE.NODE}/tdl/${tdl.toLowerCase()}`, name: tdl}));
      }
    }
  });
  ...
  Object.keys(paths).forEach((pid) => {
    ...
    path.forNids((nid) => {
      if (nodes[nid] === undefined) {
        logger.warn(`${nid} not found in path ${pid}`);
        delete paths[pid];
        return;
      }
    });
    ...
    path.forEids((eid) => {
      const edge = edges[eid];
      if (edge === undefined) {
        logger.warn(`${eid} not found in path ${pid}`);
        delete paths[pid];
        return;
      }
      for (const tag of taglib.get_tags(edge)) {
        taglib.set_tag(path, _promote_tag(tag, _TAG_SCOPE.PATH));
      }
    });
    ...
    if (bta.is_chemical(path_start)) {
      _decorate_clinical_tag(path_start);
      _decorate_role_tags(path_start);
      _decorate_otc_tag(path_start);
      // Generate tags for chemical indications
      if (trapi.is_chemical_disease_query(query_type)) {
        _decorate_indication_tag(path, nodes);
      }
    }
    // Paths inherit node tags
    for (const tag of taglib.get_tags(nodes[path.start])) {
      if (!_is_external_tag(tag)) continue;
      taglib.set_tag(path, _promote_tag(tag, _TAG_SCOPE.RESULT));
    }
    path.forInternalNids((nid) => {
      const node = nodes[nid];
      for (const tag of taglib.get_tags(node)) {
        if (!_is_external_tag(tag)) continue;
        taglib.set_tag(path, _promote_tag(tag, _TAG_SCOPE.PATH));
      }
      const node_type = node.get_specific_type();
      const tag_node_type = new taglib.Tag({id: `${_TAG_SCOPE.PATH}/pc/${node_type}`, name: node_type});
      taglib.set_tag(path, tag_node_type);
    });
    // Generate a special tag for the answer node
    _decorate_max_phase_tag(path, nodes, query_type);
    // Generate tags based on the aras for this path
    const agent_infores = path.agents;
    agent_infores.forEach((infores) => {
      const provenance = bl.infores_to_provenance(infores);
      if (cmn.is_missing(provenance)) throw new _MissingInforesError(infores);
      const tag_ara = new taglib.Tag({id: `${_TAG_SCOPE.PATH}/ara/${infores}`, name: provenance.name});
    ...
    const tag_length = new taglib.Tag({id: `${_TAG_SCOPE.PATH}/pt/${edge_count}`, name: `${edge_count} ${tag_length_description}`});
    taglib.set_tag(path, tag_length);
    path.tags = taglib.tags_to_raw_obj(path);
  });

  Object.keys(edges).forEach((eid) => {
    const edge = cmn.json_get(edges, eid);
    edge.support = edge.support.filter(pid => paths[pid] !== undefined);
    edge.tags = taglib.tags_to_raw_obj(edge);
  });
  ...
  return new Summary(meta, results, paths, nodes, edges, pubs, trials, summaryProvenance, tags, errors);
}

function _decorate_clinical_tag(node) {
  const annotations = node.annotations.chemical;
  const fda_approval = annotations.approval;
  if (!cmn.is_missing(fda_approval)) {
    if (fda_approval === 4) {
      taglib.set_tag(node, _CONSTANTS.TAGS.NODE.FDA.APPROVED);
    } else if (fda_approval === 0) {
      taglib.set_tag(node, _CONSTANTS.TAGS.NODE.FDA.NOT_APPROVED);
    } else {
      taglib.set_tag(node,
        new taglib.Tag({
          id: `${_TAG_SCOPE.NODE}/fda/${fda_approval}`,
...
function _decorate_role_tags(node) {
  const annotations = node.annotations.chemical;
  if (!cmn.is_missing(annotations.roles)) {
    for (const role of annotations.roles) {
      taglib.set_tag(node,
        new taglib.Tag({id: `${_TAG_SCOPE.NODE}/role/${role.id}`, name: cmn.titleize(role.name)}));
    }
  }
}

function _decorate_otc_tag(node) {
  const annotations = node.annotations.chemical;
  const otc_code = annotations.otc_status?.code ?? null;
  switch(otc_code) {
    case 2: taglib.set_tag(node, _CONSTANTS.TAGS.NODE.OTC.OTC); break;
    case 1: taglib.set_tag(node, _CONSTANTS.TAGS.NODE.OTC.PRESCRIPTION); break;
    case 0: taglib.set_tag(node, _CONSTANTS.TAGS.NODE.OTC.DISCONTINUED); break;
    case -2: taglib.set_tag(node, _CONSTANTS.TAGS.NODE.OTC.WITHDRAWN); break;
    case -1:
    default: taglib.set_tag(node, _CONSTANTS.TAGS.NODE.OTC.UNKNOWN);
  }
}

function _decorate_indication_tag(path, nodes) {
  ...
  if (!cmn.is_missing(indications)
      && taglib.has_tag(path_start, _CONSTANTS.TAGS.NODE.FDA.APPROVED)) {
    ...
    for (let i = 0; i < path_end_mesh_ids.length; i++) {
      if (indications_set.has(path_end_mesh_ids[i])) {
        taglib.set_tag(path_start, _CONSTANTS.TAGS.NODE.INDICATED.YES);
        return;
      }
    }
  }
  taglib.set_tag(path_start, _CONSTANTS.TAGS.NODE.INDICATED.NO);
}

function _decorate_max_phase_tag(path, nodes, query_type) {
  ...
  const path_start = nodes[path.start];
  const fda_tags = taglib.get_tags(path_start).filter(_is_fda_tag);
  ...
  if (__is_drug(path_start, highest_fda_approval)) {
    taglib.set_tag(path, _CONSTANTS.TAGS.RESULT.CHEMICAL_CLASS.DRUG);
  } else if (__is_clinical_phase(highest_fda_approval)) {
    taglib.set_tag(path, new taglib.Tag({
      id: `${_TAG_SCOPE.RESULT}/cc/phase${highest_fda_approval}`,
      name: `Phase ${highest_fda_approval} Drug`
    }));
  } else {
    taglib.set_tag(path, _CONSTANTS.TAGS.RESULT.CHEMICAL_CLASS.OTHER);
  }
  return;
...
function _conflate_gene_and_protein(node) {
  const node_type = node.get_specific_type();
  if (node_type === "Protein") {
    const gene_type = bl.tag_biolink("Gene");
    const gene_index = node.types.indexOf(gene_type);
    if (gene_index > -1) {
      node.types.splice(gene_index, 1);
      node.types.unshift(gene_type);
    }
  }
}

function getResultAnalyses(result) {
  return cmn.json_get(result, 'analyses');
}

function getResultScoringComponents(result) {
  const scoringComponents = cmn.json_get(
    result,
    'ordering_components',
    {confidence: 0, novelty: 0, clinical_evidence: 0}
  );

  const normalizedScore = cmn.json_get(result, 'normalized_score', 0);
  cmn.json_set(scoringComponents, 'normalized_score', normalizedScore);
...
const _CONSTANTS = Object.freeze({
  TAGS: {
    EDGE: {
      EVIDENCE: {
        CLINICAL: new taglib.Tag({id: `${_TAG_SCOPE.EDGE}/ev/clinical`, name: 'Clinical Trials'}),
        PUBLICATIONS: new taglib.Tag({id: `${_TAG_SCOPE.EDGE}/ev/publications`, name: 'Publications'}),
        ONTOLOGICAL: new taglib.Tag({id: `${_TAG_SCOPE.EDGE}/ev/ontology`, name: 'Accepted Ontology'}),
        OTHER: new taglib.Tag({id: `${_TAG_SCOPE.EDGE}/ev/other`, name: 'Other'})
      }
    },
    NODE: {
      FDA: {
        APPROVED: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/fda/4`, name: 'FDA Approved'}),
        NOT_APPROVED : new taglib.Tag({id: `${_TAG_SCOPE.NODE}/fda/0`, name: 'Not FDA Approved'})
      },
      INDICATED: {
        YES: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/di/ind`, name: 'Has Been in Trial'}),
        NO: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/di/not`, name: 'Has Not Been in Trial'})
      },
      OTC: {
        OTC: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/otc/t`, name: "Over the counter"}),
        PRESCRIPTION: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/otc/f`, name: "Prescription only"}),
        DISCONTINUED: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/otc/d`, name: "Discontinued"}),
        WITHDRAWN: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/otc/w`, name: "Withdrawn"}),
        UNKNOWN: new taglib.Tag({id: `${_TAG_SCOPE.NODE}/otc/o`, name: "Other"})
      }
    },
    PATH: {
      TYPE: {
        DIRECT: new taglib.Tag({id: `${_TAG_SCOPE.PATH}/pt/lkup`, name: 'Direct'}),
        INDIRECT: new taglib.Tag({id: `${_TAG_SCOPE.PATH}/pt/inf`, name: 'Indirect'})
      }
    },
    RESULT: {
      CHEMICAL_CLASS: {
        DRUG: new taglib.Tag({id: `${_TAG_SCOPE.RESULT}/cc/drug`, name: 'Drug'}),
        OTHER: new taglib.Tag({id: `${_TAG_SCOPE.RESULT}/cc/other`, name: 'Other'})
      }
    }
  }
});

class _MissingInforesError extends Error {
  constructor(infores) {
