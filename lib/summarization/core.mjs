...
class SummaryFragmentEdges {
  ...
  merge(smryFgmtEdges) {
    Object.keys(smryFgmtEdges.base).forEach((eid) => {
      const currentEdge = cmn.jsonSetDefaultAndGet(this.base, eid, new SummaryEdge(eid));
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, query_type, feature_config, errors) {
  ...
  function extendSmryPaths(paths, newPaths, agents) {
    newPaths.forEach((path) => {
      const summary_path = cmn.jsonGet(paths, path.id, false);
      if (summary_path) {
  ...
  function _extend_summary_graph_elements(objs, updates, agents, defaultValue) {
    updates.forEach((update) => {
      const obj = cmn.jsonSetDefaultAndGet(objs, update.id, defaultValue(update.id));
  ...
  function extendSmryEdges(edges, edgeFgmts, agents) {
    Object.keys(edgeFgmts.base).forEach((eid) => {
      const edge = cmn.jsonSetDefaultAndGet(edges, eid, new SummaryEdge(eid));
  ...
  function extractAndFinalizeEdges(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate', pred.gen_qualified_predicate(edge, edge.is_inverted()));
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    const seen = new Map();
    cmn.jsonUpdate(edge, 'provenance', (primary_sources) => {
      return primary_sources.map((source) => {
        const provenance = cmn.deepCopy(bl.source_to_provenance(source));
        if (!provenance) {
