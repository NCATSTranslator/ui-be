...
function genSupChain(pids, paths, edges) {
  ...
  while (remaining.length !== 0) {
    ...
    summary_path.forEids((eid) => {
      const edgeSup = edges[eid].support;
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, query_type, feature_config, errors) {
  ...
  function extractAndFinalizeEdges(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate', pred.genQualifiedPred(edge, edge.is_inverted()));
    ...
    for (const result of results) {
      ...
      root_pids.forEach((pid) => {
        ...
        summary_path.forEids((eid) => {
          ...
          is_path_indirect = is_path_indirect || edge.has_support();
        });
        let tag = null;
        if (is_path_indirect) {
      ...
      expanded_results.push({
        ...
        'subject': start,
        'drug_name': name,
        'paths': _pid_sort(root_pids, paths, _query_type_to_path_compare(query_type)),
        'object': end,
        'scores': scores[start],
        'tags': tags
      });
  ...
  Object.keys(edges).forEach((eid) => {
    const edge = cmn.jsonGet(edges, eid);
    edge.support = _pid_sort(edge.support, paths, _query_type_to_path_compare(query_type));
  });
