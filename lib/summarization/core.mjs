...
function answers_to_summary(qid, answers, max_hops) {
  ...
  const knode_summarization_rules = _make_extraction_rules(
    [
      ...
      ba.make_rule_collect_chemical_annotations(),
      ba.make_rule_collect_gene_annotations(),
      ba.make_rule_collect_disease_annoations()
    ]);
...
function getResultStartAndEnd(result, startKey, endKey) {
  ...
  function flattenBinding(result, key) {
    const nodeBinding = trapi.get_node_bindings(result, key);
    if (cmn.is_array_empty(nodeBinding)) {
...
//          if (otherNames === null
//              || (cmn.is_array_empty(otherNames.commercial) && cmn.is_array_empty(otherNames.generic))) {
//            return null;
//          }
//          return otherNames;
//        }
