...
function _decorate_clinical_tag(node) {
  ...
  const fda_approval = annotations.approval?.value ?? null;
...
function _decorate_role_tags(node) {
  const annotations = node.annotations.chemical;
  const roles = annotations.roles?.value;
  if (!cmn.is_missing(roles)) {
...
function _decorate_otc_tag(node) {
  const annotations = node.annotations.chemical;
  const otc_code = annotations.otc_status?.value?.code ?? null;
  switch(otc_code) {
...
function _decorate_indication_tag(path, nodes) {
  ...
  const path_start = nodes[path.start];
  const indications = path_start.annotations.chemical.indications?.value ?? null;
