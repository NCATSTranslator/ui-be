...
function _make_kedge_transform(eid, kedge, kedge_summarization_rules, summary_context) {
  ...
  const primary_source = trapi.get_primary_source(kedge);
  const provenance = bl.source_to_provenance(primary_source);
...
function _summary_fragments_to_summary(qid, smryFgmts, kgraph, query_type, feature_config, errors) {
  ...
  function extendSmryPubs(smryPubs, edge) {
    ...
    Object.keys(pubs).forEach((ks) => {
      ...
      pubData.forEach((pub) => {
        ...
        cmn.json_set(smryPubs, pid, new SummaryPublication(type, url, pub.src));
      });
    });
  }
  ...
  Object.keys(edges).forEach((eid) => {
    ...
    const seen = new Map();
    cmn.json_update(edge, 'provenance', (primary_sources) => {
      return primary_sources.map((source) => {
        const provenance = cmn.deep_copy(bl.source_to_provenance(source));
        if (!provenance) {
          edgeErrorReasons.push(`Found invalid source ID ${source.id} on edge ${edgeToString(edge)}`);
          return false;
        }
        if (feature_config.pull_source_records && !cmn.is_array_empty(source.records)) {
          provenance.url = source.records[0];
        }
        if (seen.has(source.id) &&
            seen.get(source.id).url === provenance.url) {
          return false;
        }
        seen.set(source.id, provenance);
        return provenance;
      }).filter(cmn.identity);
    });
  ...
  Object.keys(paths).forEach((pid) => {
    ...
    agent_infores.forEach((infores) => {
      ...
      if (cmn.is_missing(provenance)) {
        throw new _MissingInforesError(infores);
      }
