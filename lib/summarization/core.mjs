...
function _promote_edge_tag(tag) {
  ...
  Object.keys(paths).forEach((pid) => {
    ...
    path.forEids((eid) => {
      ...
      for (const tag of taglib.get_tags(edge)) {
        taglib.set_tag(path, _promote_edge_tag(tag));
      }
    });
    ...
    agent_infores.forEach((infores) => {
      ...
      const tag_ara = new taglib.Tag({id: `p/ara/${infores}`, name: provenance.name});
...
function _decorate_clinical_tag(node) {
  ...
  if (!cmn.is_missing(fda_approval)) {
    if (fda_approval === 4) {
      taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.FDA.APPROVED);
    } else if (fda_approval === 0) {
      taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.FDA.NOT_APPROVED);
    } else {
      taglib.set_tag(node,
        new taglib.Tag({
...
function _decorate_otc_tag(node) {
  ...
  switch(otc_code) {
    case 2: taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.OTC.OTC); break;
    case 1: taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.OTC.PRESCRIPTION); break;
    case 0: taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.OTC.DISCONTINUED); break;
    case -2: taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.OTC.WITHDRAWN); break;
    case -1:
    default: taglib.set_tag(node, _CONSTANTS.TAGS.RESULT.OTC.UNKNOWN);
  }
}
...
function _decorate_indication_tag(path, nodes) {
  ...
  if (!cmn.is_missing(indications)
      && taglib.has_tag(path_start, _CONSTANTS.TAGS.RESULT.FDA.APPROVED)) {
...
const _CONSTANTS = Object.freeze({
  TAGS: {
    EDGE: {
      EVIDENCE: {
        CLINICAL: new taglib.Tag({id: 'e/ev/clinical', name: 'Clinical Trials'}),
        PUBLICATIONS: new taglib.Tag({id: 'e/ev/publications', name: 'Publications'}),
        ONTOLOGICAL: new taglib.Tag({id: 'e/ev/ontology', name: 'Accepted Ontology'}),
        OTHER: new taglib.Tag({id: 'e/ev/other', name: 'Other'})
      }
    },
    PATH: {
      TYPE: {
    ...
    RESULT: {
      ...
      INDICATED: {
        YES: new taglib.Tag({id: 'r/di/ind', name: 'Has Been in Trial'}),
        NO: new taglib.Tag({id: 'r/di/not', name: 'Has Not Been in Trial'})
      },
      OTC: {
