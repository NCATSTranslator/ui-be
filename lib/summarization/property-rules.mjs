...
function make_rule_group_publications_by_knowledge_level() {
  ...
  return trapi_rules.make_rule_map_attributes({
    attr_ids: attr_ids.map(bl.tag_biolink),
    ...
    update: (edge, publications) => {
      const current = cmn.json_set_default_and_get(edge, trapi.CONSTANTS.ATTRIBUTES.ID.PUBLICATIONS, {});
      for (const publication of publications) {
        const group = cmn.json_set_default_and_get(current, publication.knowledge_level, []);
        group.push({id: publication.id, infores: publication.infores});
      }
      return edge;
    }
  });
...
function make_rule_collect_publication_supporting_text() {
  return trapi_rules.make_rule_map_attributes({
    attr_ids: [bl.tag_biolink('has_supporting_studies')],
  ...
  function __process_publication_studies(supporting_studies) {
    ...
    for (let i = 0; i < studies.length; i++) {
      ...
      let category = study.category;
      if (cmn.is_array(category)) {
        category = category[0];
      }
      if (bl.tag_biolink('Study') !== category) continue;
...
function _merge_records_list(target_key) {
  return (obj, records_list) => {
    const current_records = cmn.json_set_default_and_get(obj, target_key, {});
