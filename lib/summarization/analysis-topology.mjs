...
function _deconstruct_analysis(analysis, kgraph, aux_graphs) {
  ...
  map_support_eb.set(_CONSTANTS.ROOT_HOST, []);
  const unprocessed_edge_bindings = _abinds_to_kgbinds(trapi.getEdgeBindings(analysis)).map((eb) => {
    edge_topology.make_edge(eb, [_CONSTANTS.ROOT_HOST]);
    map_support_eb.get(_CONSTANTS.ROOT_HOST).push(eb);
    return eb;
  });
...
class AnalysisTopology {
  ...
  is_edge_root(eid) {
    return this.edge_topology.has_host(eid, _CONSTANTS.ROOT_HOST);
  }
