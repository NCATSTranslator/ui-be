...
function gen_qualified_predicate(kedge, invert = false) {
  ...
  pred = bl.sanitizeBiolinkItem(get_most_specific_predicate(kedge));
  const special_case = _gen_special_predicate(pred, qualifiers, invert);
  if (special_case) {
    return special_case;
  }
  ...
  if (invert) {
    ...
    return _combine_predicate_parts(object_qualification, bl.invertBiolinkPred(pred), subject_qualification);
  }
  ...
  return _combine_predicate_parts(subject_qualification, pred, object_qualification);
  ...
  function _combine_predicate_parts(prefix, pred, suffix) {
    if (prefix) {
      prefix += ' ';
    }
    if (suffix) {
    ...
    const qualified_predicate = `${prefix}${pred}${suffix}`;
    return qualified_predicate;
  }

  function _gen_special_predicate(pred, qualifiers, invert) {
    ...
    if (pred === 'regulates' &&
      ...
      if (invert) {
        return `is ${object_direction_qualifier} by`;
      }
