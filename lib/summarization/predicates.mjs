export {
  ...
  gen_canonical_qualified_predicate,
...
function gen_qualified_predicate(kedge, invert = false) {
  let predicate = bl.sanitize_biolink_item(trapi.get_predicate(kedge));
  let qualifiers = _get_qualifiers(kedge);
  if (!qualifiers && bl.is_deprecated_pred(predicate)) {
    [predicate, qualifiers] = bl.deprecated_pred_to_pred_and_qualifiers(predicate);
  }
  // If we don"t have any qualifiers, treat it like biolink v2 and just return
  // the base predicate
  if (!qualifiers) {
    if (invert) return bl.invert_biolink_pred(predicate);
    return predicate;
  }
  predicate = bl.sanitize_biolink_item(get_most_specific_predicate(kedge));
  ...
  if (invert) {
    ...
    return _combine_predicate_parts(object_qualification, bl.invert_biolink_pred(predicate), subject_qualification);
  }
  ...
  function _gen_qualification(type, qualifiers, prefixes) {
    ...
    const qualifier_values = qualifier_keys
      .map(key => cmn.json_get(qualifiers, `${type} ${key} qualifier`, false))
...
/**
 * Generates the fully qualified predicate for a summary edge in the canonical
 * direction, as determined by the biolink model canonical base predicate. This
 * expects a SummaryEdge from the summarization pipeline, not a raw TRAPI kedge.
 *
 * @param {SummaryEdge} summary_edge - The summary edge to generate the predicate for.
 *
 * @returns {string} - The canonical qualified predicate.
 */
function gen_canonical_qualified_predicate(summary_edge) {
  let base_predicate = bl.sanitize_biolink_item(summary_edge.predicate);
  const qualifiers = _get_qualifiers(summary_edge);
  if (!qualifiers && bl.is_deprecated_pred(base_predicate)) {
    [base_predicate] = bl.deprecated_pred_to_pred_and_qualifiers(base_predicate);
  }
  return gen_qualified_predicate(summary_edge, !bl.is_canonical_pred(base_predicate));
}
...
function _get_qualifiers(kedge) {
  try {
    ...
    trapi_qualifiers.forEach((qualifier) => {
      const qualifier_id = bl.sanitize_biolink_item(trapi.get_qualifier_id(qualifier));
      const qualifier_val = bl.sanitize_biolink_item(trapi.get_qualifier_val(qualifier));
