...
function analysis_to_analysis_graph(analysis, kgraph, aux_graphs) {
  const analysis_graph = _deconstruct_analysis(analysis, kgraph, aux_graphs);
  return _validate_analysis_graph(analysis_graph, kgraph);
}
...
function _validate_analysis_graph(analysis_graph, kgraph) {
  for (const nb of analysis_graph.node_bindings) {
    if (!trapi.hasKnode(nb, kgraph)) return false;
  }
  analysis_graph.edge_bindings = analysis_graph.edge_topology.edge_ids().filter(eb => {
    const kedge = trapi.getKedge(eb, kgraph);
  ...
  if (cmn.isArrayEmpty(analysis_graph.edge_bindings)) return false;
  analysis_graph.nb_to_outedges = _make_fn_nb_to_outedges(analysis_graph, kgraph);
  return analysis_graph;
}
