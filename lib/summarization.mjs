 ...
 */
function getQualifiedPredicate(kedge, invert = false) {
  ...
  function generateQualifiedPredicate(prefix, predicate, suffix) {
    if (prefix) {
      prefix += ' ';
    }
  ...
  function getSpecialCasePredicate(predicate, qualifiers, invert) {
  ...
  const specialCase = getSpecialCasePredicate(predicate, qualifiers, invert);
  if (specialCase) {
    return specialCase;
  }
  ...
  if (invert) {
    ...
    return generateQualifiedPredicate(objectQualification, bl.invertBiolinkPredicate(predicate), subjectQualification);
  }
  ...
  return generateQualifiedPredicate(subjectQualification, predicate, objectQualification);
}
...
function redgeToKey(redge, kgraph, doInvert = false) {
  ...
  const predicate = getQualifiedPredicate(kedge, doInvert);
...
async function summaryFragmentsToSummary(qid, summaryFragments, queryType, nodeAnnotationClient, errors) {
  ...
  function edgesToEdgesAndPublications(edges) {
    function addInverseEdge(edges, edge) {
      const invertedPredicate = getQualifiedPredicate(edge, true);
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate', getQualifiedPredicate(edge));
