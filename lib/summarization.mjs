  ...
  return makeExtractionRule(
    ...
    (publications, acc) =>
    {
      ...
      Object.keys(publications).forEach((kl) => {
        if (!currentPublications[kl]) {
          currentPublications[kl] = [];
        }

        currentPublications[kl].push(...(publications[kl]));
      });
...
function genEid(redge, kgraph, doInvert = false) {
  ...
  if (doInvert) {
    return genPid([kobject, predicate, ksubject, kl]);
  }

  return genPid([ksubject, predicate, kobject, kl]);
}

function summarizeRnode(rnode, kgraph, nodeRules, context) {
  const nid = genNid(rnode, kgraph);
  return cmn.makePair(nid,
                      nodeRules(trapi.getKnode(rnode, kgraph), context),
                      'id',
                      'transforms');
}
...
function summarizeRedge(redge, kgraph, edgeRules, context, edgeBaseIds) {
  ...
  return cmn.makePair(eid,
                      edgeRules(trapi.getKedge(redge, kgraph), context),
                      'id',
                      'transforms');
}
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  function edgesToEdgesAndPublications(edges) {
    function addInverseEdge(edges, edge) {
      ...
      const invertedEid = genPid([object, invertedPredicate, subject, kl]);
