  ...
  const nodeRules = makeExtractionRules(
    [
      ...
      aggregateAndTransformAttributes(
        [bl.tagBiolink('xref'), bl.tagBiolink('same_as')],
        'curies',
        (maybe_curie) => {return bl.isValidCurie(maybe_curie) ? maybe_curie : [];}
      ),
...
function find_id(node, predicate) {
  const curie = node.findCurie(predicate);
  if (curie) return curieLib.get_id(curie);
  return false;
}
...
const gen_chembl_url = gen_simple_url(
  node => find_id(node, curieLib.is_chembl_compound),
...
function gen_ctd_url(edge, nodes) {
  function _getCtdCurie(node) {
    for (const curie of node.curies) {
      if (curieLib.is_mesh(curie) || curieLib.is_ncbi(curie)) return curie;
    }
    return false;
  }

  function _genCtdType(curie, node) {
    if (curie && curieLib.is_ncbi(curie)) return 'gene';
  ...
  function _genCtdGeneCurie(curie) {
    return `GENE:${curieLib.get_id(curie)}`;
  }

  function _genCtdChemGeneUrl(chemCurie, geneCurie) {
      return `https://ctdbase.org/query.go?type=ixn&chemqt=equals&chem=${curieLib.gen_url_safe_curie(chemCurie)}&actionDegreeTypes=increases&actionDegreeTypes=decreases&actionDegreeTypes=affects&actionTypes=ANY&geneqt=equals&gene=${curieLib.gen_url_safe_curie(_genCtdGeneCurie(geneCurie))}&pathwayqt=equals&pathway=&taxonqt=equals&taxon=&goqt=equals&go=&sort=chemNmSort&perPage=50&action=Search`;
  }
  ...
  return `https://ctdbase.org/detail.go?type=${searchType}&acc=${curieLib.get_id(curie)}${filter}`;
}

function gen_dgidb_url(edge, nodes) {
  function _gen_dgidb_url(node) {
    let curie = node.findCurie((curie) => curieLib.is_ncbi(curie) || curieLib.is_hgnc(curie));
    if (curie) {
      return `https://dgidb.org/results?searchType=gene&searchTerms=${curieLib.gen_url_safe_curie(curie)}`;
    }
    curie = node.findCurie((curie) =>
      curieLib.is_chembl_compound(curie)
      || curieLib.is_pubchem_compound(curie)
      || curieLib.is_drugbank(curie));
    if (curie) {
      if (curieLib.is_chembl_compound(curie)) {
        curie = curieLib.remove_compound(curie);
      }
      return `https://dgidb.org/results?searchType=drug&searchTerms=${curieLib.gen_url_safe_curie(curie)}`;
    }
    return false;
  }
  return _gen_dgidb_url(nodes[edge.subject])
         || _gen_dgidb_url(nodes[edge.object])
         || false;
}

function gen_diseases_url(edge, nodes) {
  const url_template = `https://diseases.jensenlab.org/Search?query=${CONSTANTS.URL_ID_PLACEHOLDER}`;
  const get_gene_url = gen_simple_url((node) => find_id(node, curieLib.is_ensembl_ensg), url_template);
  const get_disease_url = gen_simple_url((node) => node.findCurie(curieLib.is_doid), url_template);
  return get_gene_url(edge, nodes)
         || get_disease_url(edge, nodes)
         || false;
}

const gen_do_url = gen_simple_url(
  (node) => node.findCurie(curieLib.is_doid),
  `https://disease-ontology.org/?id=${CONSTANTS.URL_ID_PLACEHOLDER}`);

const gen_drugbank_url = gen_simple_url(
  (node) => find_id(node, curieLib.is_drugbank),
  `https://go.drugbank.com/drugs/${CONSTANTS.URL_ID_PLACEHOLDER}`);

const gen_drugcentral_url = gen_simple_url(
  (node) => find_id(node, curieLib.is_drug_central),
  `https://drugcentral.org/drugcard/${CONSTANTS.URL_ID_PLACEHOLDER}`);

const gen_efo_url = gen_simple_url(
  (node) => {
    const curie = node.findCurie((curie) => {
      return curieLib.is_mondo(curie)
             || curieLib.is_chebi(curie)
             || curieLib.is_go(curie)
             || curieLib.is_ncit(curie)
             || curieLib.is_hp(curie)
             || curieLib.is_uberon(curie)
             || curieLib.is_doid(curie);
    });
    if (curie) return curie.replace(':', '_');
    return false},
  `https://purl.obolibrary.org/obo/${CONSTANTS.URL_ID_PLACEHOLDER}`);

const gen_gtopdb_url = gen_simple_url(
  (node) => find_id(node, curieLib.is_gtopdb),
  `https://www.guidetopharmacology.org/GRAC/LigandDisplayForward?ligandId=${CONSTANTS.URL_ID_PLACEHOLDER}`);

function gen_intact_url(edge, nodes) {
  const subId = find_id(nodes[edge.subject], curieLib.is_pr);
  const objId = find_id(nodes[edge.object], curieLib.is_pr);
  if (!subId && !objId) return false;
  if (subId && objId) {
    return `https://www.ebi.ac.uk/intact/search?query=id:${subId}%20AND%20id:${objId}`;
  }
  let id = subId;
  if (!subId) {
    id = objId
  }
  return `https://www.ebi.ac.uk/intact/search?query=${id}`;
}

const gen_mesh_url = gen_simple_url(
  (node) => find_id(node, curieLib.is_mesh),
  `https://meshb.nlm.nih.gov/record/ui?ui=${CONSTANTS.URL_ID_PLACEHOLDER}`);

const gen_sider_url = gen_simple_url(
  (node) => find_id(node, curieLib.is_pubchem_compound),
  `http://sideeffects.embl.de/drugs/${CONSTANTS.URL_ID_PLACEHOLDER}`);

const _KNOWLEDGE_SRC_GENERATORS = Object.freeze({
  'infores:chembl':                 gen_chembl_url,
  'infores:ctd':                    gen_ctd_url,
  'infores:dgidb':                  gen_dgidb_url,
    'infores:biothings-dgidb':      gen_dgidb_url,
  'infores:disease-ontology':       gen_do_url,
  'infores:drugbank':               gen_drugbank_url,
  'infores:drugcentral':            gen_drugcentral_url,
    'infores:automat-drug-central': gen_drugcentral_url,
  'infores:diseases':               gen_diseases_url,
    'infores:biothings-diseases':   gen_diseases_url,
  'infores:efo':                    gen_efo_url,
  'infores:gtopdb':                 gen_gtopdb_url,
  'infores:intact':                 gen_intact_url,
    'infores:automat-intact':       gen_intact_url,
  'infores:mesh':                   gen_mesh_url,
  'infores:sider':                  gen_sider_url
});
