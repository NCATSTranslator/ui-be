...
class Summary {
  constructor(meta, results, paths, nodes, edges, pubs, tags, errors) {
    this.meta = meta || {};
 ...
 */
export function answersToSummary (qid, answers, maxHops) {
  if (answers.length < 1) {
    return {};
  }
  ...
  // TODO: when query_graph isn't in he result throw an error
  ...
  return summaryFragmentsToSummary(
    qid,
    sfs,
    answersToKnowledgeGraph(answers),
    queryType,
    errors);
}
...
function answersToKnowledgeGraph(answers) {
  return trapi.getKnowledgeGraph(answers[0]); // This assumes a fully merged TRAPI message
}
...
function makeExtractionRule(key, transform, update, defaultValue) {
  return (source, context) => {
    return (target) => {
      try {
        const v = transform(source, key, context);
...
function tagAttribute(attributeId, transform) {
  return makeExtractionRule(
    ...
    (obj, key, context) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const result = attrIter.findOne([attributeId]);
...
function makeExtractionRules(rules) {
  return (source, context) => {
    return rules.map(rule => { return rule(source, context); });
  };
}
...
function getResultStartAndEnd(result, startKey, endKey) {
   ...
   *
   * @param {object} result - The TRAPI result.
   * @param {string} key - The key of the node binding.
   *
   * @returns {string[]} - The flattened list of IDs associated with the node binding.
   * @throws {NodeBindingNotFoundError} - If either of the start or end nodes are not found.
   */
...
function getSupportGraphs(kedge) {
  const supportGraphs = trapi.getSupportGraphs(kedge);
  if (supportGraphs === null) {
    return [];
  }
...
function getQualifiers(kedge) {
  try {
    ...
    trapiQualifiers.forEach((qualifier) => {
      ...
      const qualifierValue = bl.sanitizeBiolinkItem(trapi.getQualifierValue(qualifier));
      qualifiers[qualifierId] = qualifierValue;
    });
...
function getMostSpecificPredicate(kedge) {
  const qualifiers = getQualifiers(kedge);
  if (!qualifiers) {
    return trapi.getPredicate(kedge);
  }

  return qualifiers['qualified predicate'] || trapi.getPredicate(kedge);
}
 ...
 */
function genQualifiedPredicate(kedge, invert = false) {
  function genQualification(type, qualifiers, directionPrefix) {
    ...
    const prefixes = [directionPrefix, false, 'of a', 'of the', false];
  ...
  function genSubjectQualification(qualifiers, directionPrefix = false) {
    return genQualification('subject', qualifiers, directionPrefix);
  }

  function genObjectQualification(qualifiers, directionPrefix = false) {
    return genQualification('object', qualifiers, directionPrefix);
  }

  function combinePredicateParts(prefix, predicate, suffix) {
    if (prefix) {
      prefix += ' ';
    }
    ...
    const qualifiedPredicate = `${prefix}${predicate}${suffix}`;
    return qualifiedPredicate;
  }

  function genSpecialPredicate(predicate, qualifiers, invert) {
  ...
  predicate = bl.sanitizeBiolinkItem(getMostSpecificPredicate(kedge));
  const specialCase = genSpecialPredicate(predicate, qualifiers, invert);
  if (specialCase) {
    return specialCase;
  }

  if (invert) {
    const subjectQualification = genSubjectQualification(qualifiers);
    const objectQualification = genObjectQualification(qualifiers, 'has');
    return combinePredicateParts(objectQualification, bl.invertBiolinkPredicate(predicate), subjectQualification);
  }

  const subjectQualification = genSubjectQualification(qualifiers, 'has');
  const objectQualification = genObjectQualification(qualifiers);
  return combinePredicateParts(subjectQualification, predicate, objectQualification);
}
 ...
 */
function genTag(label, name, description = '') {
  return {
    'label': label,
    'description': genTagDescription(name, description)
  };
}
 ...
 */
function genTagDescription(name, description = '') {
  return {
    'name': name,
    'value': description
  };
...
function getTags(graphElement) {
  // TODO: Throw an error if the tags are not found
  return graphElement.tags;
}
 ...
 */
function addTag(summaryElement, tag) {
  summaryElement.tags[tag.label] = tag.description;
  return summaryElement;
}

function mergeTags(summaryTarget, summarySource, func = null) {
  let sourceTags = Object.keys(summarySource.tags);
  if (func !== null) {
    sourceTags = sourceTags.filter(func);
  }

  sourceTags.forEach((k) => {summaryTarget.tags[k] = summarySource.tags[k];});
...
function genMaxPhaseTag(node, queryType) {
  ...
  const fdaTags = Object.keys(getTags(node)).filter((tag) => { return tag.startsWith('fda'); });
  ...
  if (highestFdaApproval === 0) return genTag('cc:other', 'Other');
  if (isDrug(node, highestFdaApproval)) return genTag('cc:drug', 'Drug');
  if (isClinicalPhase(highestFdaApproval)) return genTag(`cc:phase${highestFdaApproval}`, `Phase ${highestFdaApproval} Drug`);
  return genTag(`cc:other`, `Other`);
}
...
function analysisToRgraph(analysis, kgraph, auxGraphs) {
  ...
  while (!cmn.isArrayEmpty(unprocessedEdgeBindings) || !cmn.isArrayEmpty(unprocessedSupportGraphs)) {
    while (!cmn.isArrayEmpty(unprocessedEdgeBindings)) {
      ...
      const edgeSupportGraphs = getSupportGraphs(kedge);
...
function redgeToKey(redge, kgraph, doInvert = false) {
  ...
  const predicate = genQualifiedPredicate(kedge, doInvert);
...
class SummaryPath {
  constructor(subgraph, supportingAgents) {
  ...
  get nodeCount() {
    ...
    return Math.floor(this.subgraph.length/2)+1;
  }

  get edgeCount() { return (this.subgraph.length-1)/2; }
  get graph() { return this.subgraph; }
  get start() { return this.subgraph[0]; }
  get end() { return this.subgraph[this.subgraph.length-1]; }

  nid(i) { return this.subgraph[i*2]; }
  forNids(func) { this._forIds(func, 0); }
  eid(i) { return this.subgraph[(i*2)+1]; }
  forEids(func) { this._forIds(func, 1); }

  _forIds(func, start) {
    for (let i = start; i < this.subgraph.length; i+=2) {
      func(this.subgraph[i]);
    }
  }
}

function summaryPathCompare(summaryPath1, summaryPath2) {
  const len1 = summaryPath1.length;
  const len2 = summaryPath2.length;
  if (len1 === len2) {
    for (let i = 0; i < summaryPath1.nodeCount; i++) {
      if (summaryPath1.nid(i) < summaryPath2.nid(i)) return -1;
      else if (summaryPath1.nid(i) > summaryPath2.nid(i)) return 1;
    }
...
function summaryPathFromPid(paths, pid) {
  return cmn.jsonGet(paths, pid);
}

function pidSort(pids, paths) {
  function pidCompare(pid1, pid2) {
    const summaryPath1 = summaryPathFromPid(paths, pid1);
    const summaryPath2 = summaryPathFromPid(paths, pid2);
    return summaryPathCompare(summaryPath1, summaryPath2);
  }
...
function summaryFragmentsToSummary(qid, summaryFragments, kgraph, queryType, errors) {
  ...
  function extendSummaryPaths(paths, newPaths, agents) {
    newPaths.forEach((path) => {
      const summaryPath = cmn.jsonGet(paths, path.key, false);
      if (summaryPath) {
        summaryPath.extendSupportingAgents(agents);
        return;
      }

      cmn.jsonSet(paths, path.key, new SummaryPath(path.path, agents));
    });
  }
  ...
  function edgesToEdgesAndPublications(edges) {
    function addInverseEdge(edges, edge) {
      const invertedPredicate = genQualifiedPredicate(edge, true);
      ...
      const unqualifiedInvertedPredicate = bl.invertBiolinkPredicate(getMostSpecificPredicate(edge));
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate_url', bl.predicateToUrl(getMostSpecificPredicate(edge)));
      cmn.jsonSet(edge, 'predicate', genQualifiedPredicate(edge));
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors) {
    function isRootPath(pid) {
      const summaryPath = summaryPathFromPid(paths, pid);
      return summaryPath.edgeCount === 1 && edges[summaryPath.eid(0)].isRootPath;
    }

    function supportChainIncludesPid(rootPids, paths, edges, pid) {
      if (isRootPath(pid)) return true;
      ...
      while (pids.length !== 0) {
        ...
        const summaryPath = summaryPathFromPid(paths, validPid);
        summaryPath.forEids((edge) => {
          const edgeSupport = edges[edge].support;
    ...
    for (const result of results) {
      ...
      const summaryPath = summaryPathFromPid(paths, rootPids[0]);
      const start = summaryPath.start;
      const startNames = cmn.jsonGetFromKpath(nodes, [start, 'names']);
      const end = summaryPath.end;
      const tags = {};
      pids.forEach((pid) => {
      ...
      rootPids.forEach((pid) => {
        const summaryPath = summaryPathFromPid(paths, pid);
        let tag = 'pt:inf';
        if (!cmn.isArrayEmpty(edges[summaryPath.eid(0)].supportPaths)) {
          usedTags[tag] = genTagDescription('Inferred');
          tags[tag] = null;
        } else {
          tag = 'pt:lkup';
          usedTags[tag] = genTagDescription('Lookup');
          tags[tag] = null;
        }
      });
  ...
  const meta = new SummaryMetadata(qid, cmn.distinctArray(summaryFragments.map((sf) => {
    return sf.agents;
  }).flat()));
  ...
  try {
        ...
        tagAttribute(
          ...
          (annotations) => {
            ...
            if (fdaApproval === null) {
              ...
              const tags = [];
              if (fdaApproval > 0) {
                tags.push(genTag(`fda:${fdaApproval}`, `Clinical Trial Phase ${fdaApproval}`));
              } else {
                tags.push(genTag('fda:0', 'Not FDA Approved'));
              }

              return tags;
            } else {
              return [genTag(`fda:${fdaApproval}`, `FDA Approved`)];
            }
          }
        ),
        tagAttribute(
          ...
          (annotations, context) => {
            ...
            return chebiRoles.map((role) => { return genTag(`role:${role.id}`, cmn.titleize(role.name))});
          }
        ),
        renameAndTransformAttribute(
          'biothings_annotations',
    ...
    results.forEach((result) => {
        ...
        pids.forEach((pid) => {
          const summaryPath = summaryPathFromPid(paths, pid);
          resultNodes.add(summaryPath.start);
        });
      });
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      cmn.objRemoveDuplicates(summaryPath);
      ...
      if (trapi.isChemicalDiseaseQuery(queryType)) {
        ...
        if (start.indications !== undefined) {
          ...
          if (indicatedFor) {
            indicationTag = genTag('di:ind', 'In a clinical trial for indicated disease');
          } else {
            indicationTag = genTag('di:not', 'Not in a clinical trial for indicated disease');
          }
      ...
      mergeTags(summaryPath, nodes[summaryPath.start]);
      summaryPath.forNids((nid) => {
        const node = nodes[nid];
        mergeTags(summaryPath, node, (tag) => !isResultTag(tag));
        const nodeType = node.type;
        const typeTag = genTag(`pc:${nodeType}`, nodeType);
        addTag(summaryPath, typeTag);
      });

      // Generate a special tag for the answer node
      const maxPhaseTag = genMaxPhaseTag(nodes[summaryPath.start], queryType);
      if (maxPhaseTag) {
        addTag(summaryPath, maxPhaseTag);
      }

      // Generate tags based on the aras for this path
      const agentInfores = summaryPath.agents;
      agentInfores.forEach((infores) => {
        const araTag = genTag(`ara:${infores}`, inforesToName(infores));
        addTag(summaryPath, araTag);
      });

      // Generate tags for number of connections
      const edgeCount = summaryPath.edgeCount
      let tagDescription = 'Connections';
      if (edgeCount === 1) {
        tagDescription = 'Connection';
      }

      const lengthTag = genTag(`pt:${edgeCount}`, `${edgeCount} ${tagDescription}`);
      addTag(summaryPath, lengthTag);
    });

    // Remove PIDs that are no longer valid from results and edge support
    Object.keys(edges).forEach((k) => {
      const edge = cmn.jsonGet(edges, k);
      edge.supportPaths = edge.supportPaths.filter(pid => paths[pid] !== undefined);
      pidSort(edge.supportPaths, paths);
    });

    results.forEach((r) => {
      r.paths = r.paths.filter(pid => paths[pid] !== undefined);
    });
    ...
    return new Summary(meta, results, paths, nodes, edges, publications, tags, errors);
  }
}
