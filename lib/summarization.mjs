...
function answersToQueryTemplate(answers) {
  ...
  const trapiAnswer = getTrapiAnswer(answers[0]);
  return trapi.messageToQueryTemplate(trapiAnswer); // This assumes a fully merged TRAPI message
}

function answersToKgraph(answers) {
  const trapiAnswer = getTrapiAnswer(answers[0]);
  return trapi.getKgraph(trapiAnswer); // This assumes a fully merged TRAPI message
}
...
function answersToSmryFgmts(answers, nodeRules, edgeRules, maxHops) {
  ...
  answers.forEach((answer) => {
    const trapiAnswer = getTrapiAnswer(answer);
    const results = trapi.getResults(trapiAnswer);
    if (!results) {
      // TODO: Add warning
      return;
    }
    ...
    const kgraph = trapi.getKgraph(trapiAnswer);
    const auxGraphs = trapi.getAuxGraphs(trapiAnswer);
    const [startKey, endKey] = trapi.messageToEndpoints(trapiAnswer);
...
function getTrapiAnswer(answer) {
  return answer.trapi;
}
