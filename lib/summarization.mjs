...
function genPaths(start, ends, max_path_len, map_rnode_outedges) {
  const finalPaths = [];
  ...
  while (!cmn.isArrayEmpty(pathsLeft)) {
    ...
    const outEdges = map_rnode_outedges(pathHead);
    if (pathLen === max_path_len-2) {
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      ...
      try {
        ...
        const rgraphPaths = genPaths(start, ends, max_path_len, map_rnode_outedges); // C!
        const [normalizedPaths, edgeBases] = finalizePaths(rgraphPaths, rgraph.edge_topology, kgraph);
        const analysisCxt = {
          agent: agent,
          errors: errors
        };
