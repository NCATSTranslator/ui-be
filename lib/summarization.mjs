...
function genMaxPhaseTag(node, queryType) {
  ...
  if (highestFdaApproval === 0) return makeTag('cc:other', 'Other');
  if (isDrug(node, highestFdaApproval)) return makeTag('cc:drug', 'Drug');
  if (isClinicalPhase(highestFdaApproval)) return makeTag(`cc:phase${highestFdaApproval}`, `Phase ${highestFdaApproval} Drug`);
  return makeTag(`cc:other`, `Other`);
}
...
function genEid(redge, kgraph, doInvert = false) {
  ...
  if (doInvert) {
    return genPid([kobject, predicate, ksubject, knowledgeLevel]);
  }

  return genPid([ksubject, predicate, kobject, knowledgeLevel]);
}
...
function summarizeRedge(redge, kgraph, edgeRules, context, edgeBaseIds) {
...
function answersToSmryFgmts(answers, nodeRules, edgeRules, maxHops) {
    ...
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      function finalizePaths(rgraphPaths, edgeMappings, kgraph) {
        ...
        for (const path of normalizedPaths) {
          ...
          pathToSupportGraph[genPid(path)] = gids;
        }
  ...
  answers.forEach((answer) => {
    ...
    const kgraph = trapi.getKnowledgeGraph(answer);
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  function fgmtPathsToResultsAndPaths(fgmtPaths) {
    ...
    fgmtPaths.forEach((path) => {
      ...
      results.push(cmn.makePair(path[0], pid, 'start', 'pid'));
  ...
  function edgesToEdgesAndPublications(edges) {
    function addInverseEdge(edges, edge) {
      ...
      const invertedEid = genPid([object, invertedPredicate, subject, knowledgeLevel]);
    ...
    Object.values(edges).forEach((edge) => {
      ...
      Object.keys(edgePublications).forEach((kl) => {
        edgePublications[kl] = edgePublications[kl].map((pub) => {
  ...
  try {
    ...
    const nodeUpdates = Object.keys(nodes).map((nid) => {
      return summarizeRnode(nid, kgraph, nodeRules, annotationContext);
    });

    const resultNodeUpdates = [...resultNodes].map((nid) => {
      return summarizeRnode(nid, kgraph, resultNodeRules, annotationContext);
    });
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      if (trapi.isChemicalDiseaseQuery(queryType)) {
        ...
        if (start.indications !== undefined) {
          ...
          if (indicatedFor) {
            indicationTag = makeTag('di:ind', 'In a clinical trial for indicated disease');
          } else {
            indicationTag = makeTag('di:not', 'Not in a clinical trial for indicated disease');
          }
      ...
      smryPath.forNids((nid) => {
        ...
        const typeTag = makeTag(`pc:${nodeType}`, nodeType);
      ...
      agentInfores.forEach((infores) => {
        const araTag = makeTag(`ara:${infores}`, inforesToName(infores));
      ...
      const lengthTag = makeTag(`pt:${edgeCount}`, `${edgeCount} ${tagDescription}`);
