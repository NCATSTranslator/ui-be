 ...
 */
function addTag(smryElement, tag) {
  smryElement.tags[tag.label] = tag.description;
  return smryElement;
}
...
class SummaryFragmentEdges {
  ...
  merge(smryFgmtEdges) {
    Object.keys(smryFgmtEdges.base).forEach((edgeKey) => {
      const currentEdge = cmn.jsonSetDefaultAndGet(this.base, edgeKey, new SummaryEdge());
      currentEdge.merge(smryFgmtEdges.base[edgeKey]);
    });
...
class SummaryEdge {
  ...
  merge(smryEdge) {
    this.extendSupportingAgents(smryEdge.aras);
    this.extendSupportPaths(smryEdge.supportPaths);
    ...
    this.predicateUrl = this.predicateUrl || smryEdge.predicateUrl;
  ...
  function resultToSmryFgmt(result, kgraph, auxGraphs, startKey, endKey, errors) {
    ...
    try {
      ...
      if (!resultSmryFgmt.isEmpty()) {
        ...
        resultSmryFgmt.pushScore(resultKey, scoringComponents);
      }
...
function smryPathFromPid(paths, pid) {
  return cmn.jsonGet(paths, pid);
}

function pidSort(pids, paths) {
  function pidCompare(pid1, pid2) {
    const smryPath1 = smryPathFromPid(paths, pid1);
    const smryPath2 = smryPathFromPid(paths, pid2);
    return smryPathCompare(smryPath1, smryPath2);
  }
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  function fgmtPathsToResultsAndPaths(fgmtPaths) {
  ...
  function extendSmryPaths(paths, newPaths, agents) {
    newPaths.forEach((path) => {
      const smryPath = cmn.jsonGet(paths, path.key, false);
      if (smryPath) {
        smryPath.extendSupportingAgents(agents);
        return;
      }
  ...
  function extendSmryGraphElement(objs, updates, agents, defaultValue) {
  ...
  function extendSmryNodes(nodes, nodeUpdates, agents) {
    extendSmryGraphElement(nodes, nodeUpdates, agents, () => new SummaryNode());
  }

  function extendSmryEdges(edges, edgeFgmts, agents) {
    Object.keys(edgeFgmts.base).forEach((k) => {
      const edge = cmn.jsonSetDefaultAndGet(edges, k, new SummaryEdge());
      edge.merge(edgeFgmts.base[k]);
    });

    extendSmryGraphElement(edges, edgeFgmts.updates, agents, () => new SummaryEdge());
  }
  ...
  function extendSmryPublications(publications, edge)
  {
  ...
  function edgesToEdgesAndPublications(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      extendSmryPublications(publications, edge);
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors) {
    function isRootPath(pid) {
      const smryPath = smryPathFromPid(paths, pid);
      return smryPath.edgeCount === 1 && edges[smryPath.eid(0)].isRootPath;
    }
    ...
    function supportChainIncludesPid(rootPids, paths, edges, pid) {
      ...
      while (pids.length !== 0) {
        ...
        const smryPath = smryPathFromPid(paths, validPid);
        smryPath.forEids((edge) => {
    ...
    for (const result of results) {
      ...
      const smryPath = smryPathFromPid(paths, rootPids[0]);
      ...
      rootPids.forEach((pid) => {
        const smryPath = smryPathFromPid(paths, pid);
        let tag = 'pt:inf';
        if (!cmn.isArrayEmpty(edges[smryPath.eid(0)].supportPaths)) {
  ...
  smryFgmts.forEach((sf) => {
    ...
    const [newResults, newPaths] = fgmtPathsToResultsAndPaths(sf.paths);
  ...
  try {
    ...
    extendSmryErrors(errors, annotationContext.errors);
  }
  catch (err) {
    console.error(err);
  }
    ...
    Object.keys(paths).forEach((pid) => {
      ...
      mergeTags(smryPath, nodes[smryPath.start]);
      smryPath.forNids((nid) => {
        const node = nodes[nid];
        mergeTags(smryPath, node, (tag) => !isResultTag(tag));
