...
function getSupText() {
  ...
  return makeExtractionRule(
    ...
    (obj, key) => {
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const supTextAttributes = attrIter.findAll([searchAttrId]);
      const supText = {};
      supTextAttributes.forEach(attribute => {
...
function analysisToRgraph(analysis, kgraph, auxGraphs) {
  ...
  while (!cmn.isArrayEmpty(unprocessedEdgeBindings) || !cmn.isArrayEmpty(unprocessedSupGraphs)) {
    ...
    while (!cmn.isArrayEmpty(unprocessedSupGraphs)) {
      const gid = unprocessedSupGraphs.pop();
      if (supGraphs.has(gid)) {
        continue;
      }
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  function edgesToEdgesAndPubs(edges) {
    ...
    Object.values(edges).forEach((edge) => {
      ...
      const supText = cmn.jsonGet(edge, 'supporting_text', {});
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors) {
    ...
    function supChainIncludesPid(rootPids, paths, edges, pid) {
      ...
      while (pids.length !== 0) {
        ...
        smryPath.forEids((edge) => {
          const edgeSup = edges[edge].supPaths;
          if (edgeSup.includes(pid)) return true;
          pids.push(...edgeSup.filter((spid) => !seenPids.has(spid)));
        });
      }
    ...
    for (const result of results) {
      ...
      pids.forEach((pid) => {
        ...
        if (supChainIncludesPid(rootPids, paths, edges, pid)) {
      ...
      rootPids.forEach((pid) => {
        const smryPath = smryPathFromPid(paths, pid);
        let tag = 'pt:inf';
        if (!cmn.isArrayEmpty(edges[smryPath.eid(0)].supPaths)) {
