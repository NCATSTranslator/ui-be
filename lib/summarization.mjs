...
/**
 * For a path, determines if the current index points to a node.
 *
 * @param {number} index - The index to check.
 *
 * @returns {boolean} - True if the index points to a node, false otherwise.
 */
 ...
 */
function generateQualifiedPredicate(kedge, invert = false) {
  function generateQualification(type, qualifiers, directionPrefix) {
  ...
  function generateSubjectQualification(qualifiers, directionPrefix = false) {
    return generateQualification('subject', qualifiers, directionPrefix);
  }

  function generateObjectQualification(qualifiers, directionPrefix = false) {
    return generateQualification('object', qualifiers, directionPrefix);
  }
  ...
  function generateSpecialPredicate(predicate, qualifiers, invert) {
  ...
  const specialCase = generateSpecialPredicate(predicate, qualifiers, invert);
  if (specialCase) {
    return specialCase;
  }

  if (invert) {
    const subjectQualification = generateSubjectQualification(qualifiers);
    const objectQualification = generateObjectQualification(qualifiers, 'has');
    return combinePredicateParts(objectQualification, bl.invertBiolinkPredicate(predicate), subjectQualification);
  }

  const subjectQualification = generateSubjectQualification(qualifiers, 'has');
  const objectQualification = generateObjectQualification(qualifiers);
...
function redgeToKey(redge, kgraph, doInvert = false) {
  ...
  const predicate = generateQualifiedPredicate(kedge, doInvert);
...
async function summaryFragmentsToSummary(qid, summaryFragments, queryType, nodeAnnotationClient, errors) {
  ...
  function edgesToEdgesAndPublications(edges) {
    function addInverseEdge(edges, edge) {
      const invertedPredicate = generateQualifiedPredicate(edge, true);
    ...
    Object.values(edges).forEach((edge) => {
      ...
      cmn.jsonSet(edge, 'predicate', generateQualifiedPredicate(edge));
