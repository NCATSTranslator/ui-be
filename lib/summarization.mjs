...
function gen_chembl_url(edge, nodes) {
  function _get_chembl_id(node) {
  ...
  const subject = nodes[edge.subject];
  const object = nodes[edge.object];
  const sub_id = _get_chembl_id(subject);
  const obj_id = _get_chembl_id(object);
  if (!sub_id && !obj_id ) return false;
  let id = obj_id;
  if(sub_id) {
    id = sub_id;
  }
  return `https://ebi.ac.uk/chembl/explore/compound/${id}#DrugIndications`;
}
...
const _KNOWLEDGE_SRC_GENERATORS = Object.freeze({
  'infores:ctd': genCtdUrl,
  'infores:chembl': gen_chembl_url,
  'infores:disease-ontology': gen_simple_url(
    (node) => node.findCurie(curieLib.isDoid),
    'https://disease-ontology.org/?id={{ID}}')
});
