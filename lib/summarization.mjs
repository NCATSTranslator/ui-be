...
function getPubs() {
  ...
  return makeExtractionRule(
    ...
    (obj, key, context) => {
      const pubs = {};
      const attrIter = new trapi.AttributeIterator(obj[key]);
      const pubAttrs = attrIter.findAll(pubIds);
      if (cmn.isArrayEmpty(pubAttrs)) return pubs;
...
function genQualifiedPredicate(kedge, invert = false) {
  ...
  let predicate = bl.sanitizeBiolinkItem(trapi.getPred(kedge));
  ...
  if (invert) {
    const subQualification = genSubQualification(qualifiers);
    const objQualification = genObjQualification(qualifiers, 'has');
    return combinePredicateParts(objQualification, bl.invertBiolinkPredicate(predicate), subQualification);
  }

  const subQualification = genSubQualification(qualifiers, 'has');
  const objQualification = genObjQualification(qualifiers);
  return combinePredicateParts(subQualification, predicate, objQualification);
}
...
function genRgraph(rnodes, redges, edgeMappings, kgraph) {
  ...
  rgraph.edges = redges.filter(redge => {
    ...
    return bl.isBiolinkPredicate(trapi.getPred(kedge));
  });
...
function genEid(redge, kgraph, doInvert = false) {
  ...
  if (doInvert) {
    return genPid([kobj, predicate, sub, kl]);
  }

  return genPid([sub, predicate, kobj, kl]);
}
...
function smryFgmtsToSmry(qid, smryFgmts, kgraph, queryType, errors) {
  ...
  function extendSmryPubs(smryPubs, edge) {
    ...
    Object.keys(pubs).forEach((ks) => {
      ...
      pubData.forEach((pub) => {
        ...
        cmn.jsonSet(smrtPubs, pid, new SummaryPublication(type, url, pub.source));
      });
    });
  }

  function edgesToEdgesAndPubs(edges) {
    function addInverseEdge(edges, edge) {
      ...
      const invertedEid = genPid([obj, invertedPredicate, sub, kl]);
  ...
  [edges, pubs] = edgesToEdgesAndPubs(edges);
  ...
  try {
    ...
    return new Summary(meta, results, paths, nodes, edges, pubs, tags, errors);
  }
}
