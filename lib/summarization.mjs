...
/**
 * For a path, get all summary nodes
 *
 * @param {string[]} path - Path to get nodes from.
 * @param {object} nodeSummary - Node summary.
 *
 * @returns {string[]} - Node IDs in the path.
 */
function getPathNodes(path, nodeSummary) {
  function isNode(index) {
    return index % 2 === 0;
  }

  return (path.subgraph
    .filter((node, i) => (isNode(i)))
    .map((nodeId) => nodeSummary[nodeId])
    .filter(cmn.identity));
}
...
async function summaryFragmentsToSummary(qid, summaryFragments, kgraph, queryType, errors) {
  ...
  try {
    ...
    Object.keys(paths).forEach((pk) => {
      ...
      path.tags = {};
      getPathNodes(path, nodes).forEach((node, i) => {
        mergeTags(path, node); // Paths should take all node tags
        // Generate a special tag when the node is the answer node
        if (i === 0) {
          const maxPhaseTag = genMaxPhaseTag(node, queryType);
          if (maxPhaseTag !== null) {
            addTag(path, maxPhaseTag);
          }
        }
      ...
      const lengthTag = genTag(`pt:${pathLength}`, `${pathLength} ${tagDescription}`);
      addTag(path, lengthTag);
    });
    ...
    Object.keys(edges).forEach((k) => {
      ...
      pathSort(edge.supportPaths, paths);
    });
