...
class SummaryEdge {
  ...
  merge(smryEdge) {
    if (this.type && smryEdge.type && this.type !== smryEdge.type) {
      throw Error(`Error: Merge on edges of different types: ${this.type} and ${smryEdge.type}`);
    }
    ...
    function analysisToSmryFgmt(analysis, kgraph, auxGraphs, start, ends) {
      function finalizePaths(rgraphPaths, edgeMappings, kgraph) {
        ...
        for (const edge of Object.keys(normalizedMappings)) {
          ...
          s_edge.isRootPath = normalizedMappings[edge].partOf.includes('root');
