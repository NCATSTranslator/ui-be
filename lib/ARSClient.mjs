...
class ARSClient {
  constructor(clientId, apiKey, origin, getPath, postPath, retainPath, subscribePath, unsubscribePath, useARSMerging, completeCodes=[200,206,444], runningCodes=[202]) {
    ...
    this.subscribeURL = `${origin}${subscribePath}`;
    this.unsubscribeURL = `${origin}${unsubscribePath}`;
  ...
  async _collectMergedResults(pkey, fetchCompleted=false) {
    ...
    // Begin temporary hack for pathfinder
    if (mergedVersionsList === 'None' && arsResp.status === 'Done') {
      for (const child of arsResp.children) {
        if (child.result_count === 1 &&
            child.code === 422 &&
            child.actor.inforesid === 'infores:arax') {
          logger.debug('Process pathfinder result');
          const arax_data = (await this._fetchMessage(child.message))[1];
          return {
            pk: pkey,
            completed: [{
              agent: 'ara-arax',
              ...
              status: 'Done',
              code: '200',
              data: arax_data.fields.data.message
            }],
            running: [],
            errored: [],
            meta: meta
    ...
    // End temporary hack for pathfinder
    ...
    const parentStatus = arsResp.status;
    const message = {
      uuid: pkey,
      agent: arsResp.actor.agent,
      data: []
    };
  ...
  async _collectMergedResults_old(pkey, statusCheck = false) {
    // Get the top level message from the ARS. This contains the list of currently
    // merged PKs.
