...
class ARSClient {
  constructor(ars_config, api_key, completeCodes=[200,206,444], runningCodes=[202]) {
    ...
    this.notificationURL = `${origin}${ars_config.notification_uri}`;
  ...
  async postQuery(query) {
    ...
    return cmn.send_recv_json2(this.postURL, 'POST', {}, query)
  }

  async retainQuery(pkey) {
    return cmn.send_recv_http2(`${this.retainURL}/${pkey}`, 'POST', {},
  ...
  async unsubscribeQuery(pkey) {
    return this.unsubscribeQueries([pkey]);
  }

  async get_notification_statuses(pks) {
    const headers = {};
    const body = { "pks": pks };
    return cmn.send_recv_json2(this.notificationURL, "POST", headers, body);
  }
  ...
  async _fetchMessage(uuid, doTrace=false, compress=false) {
    ...
    return cmn.send_recv_json2(url, 'GET', headers, null, compress);
  }
  ...
  _queryCallbackRequest(url, pkeys) {
    ...
    const headers = {};
    headers[_CUSTOM_HEADERS.X_EVENT_SIG] = cmn.generate_hmac_signature(JSON.stringify(body), this.apiKey);
    return cmn.send_recv_json2(url, 'POST', headers, body);
  }
}
