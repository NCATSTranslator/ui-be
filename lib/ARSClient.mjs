...
class ARSClient {
  ...
  processQueryUpdate(arsUpdate) {
    ...
    switch (eventType) {
      case _CONSTANTS.CALLBACK_EVENT.MV_AVAILABLE:
        update.aras = cmn.jsonGet(arsUpdate, 'merged_versions_list').map(idAraPair => idAraPair[1]);
        update.aras.reverse();
        if (cmn.jsonGet(arsUpdate, 'complete')) {
          update.status = cmn.CONSTANTS.QUERY_STATUS.COMPLETE;
        } else {
          update.status = cmn.CONSTANTS.QUERY_STATUS.RUNNING;
        }
        break;
      case _CONSTANTS.CALLBACK_EVENT.MV_BEGUN:
      case _CONSTANTS.CALLBACK_EVENT.ARA_COMPLETE:
        break; // We can ignore these events for now
      case _CONSTANTS.CALLBACK_EVENT.ERROR:
        update.status = cmn.CONSTANTS.QUERY_STATUS.ERROR;
        break;
      default:
        throw new Error(`Unknown event type from ARS: ${eventType}`);
    }
    return update;
  }
