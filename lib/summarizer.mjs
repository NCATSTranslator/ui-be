...
function answersToQueryType(answers) {
  ...
  return trapi.messageToQueryType(answers[0]);
}
...
class SummaryEdge {
  ...
  merge(summaryEdge) {
    ...
    this.publications.push(...summaryEdge.publications);
    return this
  }
}
...
async function summaryFragmentsToSummary(qid, summaryFragments, queryType, nodeAnnotationClient, errors) {
  ...
  function edgesToEdgesAndPublications(edges) {
    ...
    Object.values(edges).forEach((edge) => {
        ...
        edge.predicateUrl = bl.predicateToUrl(getSpecificPredicate(edge));
  ...
  const metadataObject = makeMetadataObject(qid, cmn.distinctArray(summaryFragments.map((sf) => {
    return sf.agents;
  }).flat()));
