 ...
 * @param {object} fallback - The default value to use if the extraction fails.
 *
 * @returns {function} - The extraction rule.
 */
function makeExtractionRule(key, transform, update, fallback) {
  return (obj, context) => {
    return (acc) => {
      try {
...
function getProperty(key) {
  return makeExtractionRule(
    key,
    cmn.identity,
    (v, acc) => { return cmn.jsonSet(acc, key, v); },
    null);
}
...
function aggregateProperty(key, kpath) {
  return aggregateAndTransformProperty(key, kpath, cmn.identity);
}
