...
export function answersToSummary (qid, answers, maxHops, nodeAnnotationClient) {
  ...
  const edgeRules = makeExtractionRules(
    [
      transformPropertyRule('predicate', (obj, key) => bl.sanitizeBiolinkItem(cmn.jsonGet(obj, key))),
...
function transformPropertyRule(key, transform) {
  return makeExtractionRule(
    key,
    transform,
    (v, acc) => { return cmn.jsonSetFromKpath(acc, [key], v); },
    null);
}
...
function aggregateAndTransformProperty(key, kpath, transform) {
  return makeExtractionRule(
    ...
    transform,
    (v, acc) => {
      const property = cmn.jsonSetDefaultAndGet(acc, kpath, []);
      const sanitizedValue = cmn.isArray(v) ? v : [v];
      property.push(...sanitizedValue);
      return acc;
    },
    []);
}
...
function noAttributes(attributes) {
...
class AttributeIterator {
  ...
  findOne(searchIds, sentinel = false) {
    while (this.hasNext()) {
      const attribute = this.attributes[this.index++];
      if (searchIds.includes(trapi.getAttributeId(attribute))) {
        return trapi.getAttributeValue(attribute);
      }
    }
  ...
  findAll(searchIds, sentinel = false) {
    const result = [];
    while (this.hasNext()) {
      const value = findOne(searchIds, sentinel);
      if (value === sentinel) {
        break;
      }
...
function renameAndTransformAttribute(attributeId, kpath, transform, sentinel = null) {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key) => {
      const attrIter = new AttributeIterator(obj[key]);
      const attrValue = attrIter.findOne(attributeId, sentinel);
...
function aggregateAndTransformAttributes(attributeIds, accKey, transform, sentinel = null) {
  return makeExtractionRule(
    trapi.CONSTANTS.GRAPH.ATTRIBUTES.KEY,
    (obj, key, context) => {
      const attrIter = new AttributeIterator(obj[key]);
      const result = attrIter.findAll(attributeIds, sentinel);
      return result.map((v) => { return transform(v, context); });
    },
    (v, obj) => {
      const currentValue = cmn.jsonSetDefaultAndGet(obj, accKey, []);
      currentValue.push(...v);
      return obj;
    },
    []);
  ...
  return makeExtractionRule(
    ...
    (obj, key, context) => {
      ...
      for (const attribute of attributes) {
        if (attributeId === trapi.getAttributeId(attribute)) {
          return transform(trapi.getAttributeValue(attribute), context);
        }
      }
  ...
  return makeExtractionRule(
    ...
    (obj, key) => {
      ...
      attributes.forEach(attribute => {
        const innerAttrs = trapi.getAttributeId(attribute) === searchAttrId ? trapi.getAttributes(attribute) : [];
...
function getPublications() {
  ...
  return makeExtractionRule(
    ...
    (obj, key, context) => {
      ...
      attributes.forEach(attribute => {
        let v = (publicationIds.includes(trapi.getAttributeId(attribute))) ? trapi.getAttributeValue(attribute) : [];
        v = cmn.isArray(v) ? v : [v];
