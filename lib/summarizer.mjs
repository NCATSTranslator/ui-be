...
// TODO: add last minute changes from Octopus release
  ...
  findOne(searchIds, sentinel = null) {
    while (this.hasNext()) {
      ...
      if (searchIds.includes(trapi.getAttributeId(attribute))) {
        const v = trapi.getAttributeValue(attribute);
        if (v) return v;
      }
    }
...
function redgeToKey(redge, kgraph, doInvert = false) {
  ...
  const knowledgeLevel = bl.inforesToProvenance(trapi.getPrimarySource(kedge)).knowledge_level;
  if (doInvert) {
