...
function _get_drug_indications(annotation) {
  ...
  for (const entry of approval_entries) {
    ...
    const curie = _clinical_approval_disease_curie(_get(entry, _CONSTANTS.CHEMICAL.CLINICAL_APPROVAL.DISEASE));
    if (!cmn.is_missing(curie)) {
      indications.add(curie);
    }
  }
  if (indications.size === 0) return _empty_section();
  return make_section([...indications], [make_source(SOURCES.DRUG_APPROVALS)]);
}

function _clinical_approval_disease_curie(disease) {
  if (cmn.is_missing(disease)) return null;
  for (const [key, value] of Object.entries(disease)) {
    if (key !== _CONSTANTS.CHEMICAL.CLINICAL_APPROVAL.DISEASE_NAME) return value;
  }
  return null;
}
