...
function make_rule_collect_chemical_annotations() {
  return _make_annotation_rule([
    [_get_chemical_names, _assign_section("other_names")],
...
function make_section(value, sources) {
...
function _get_from_sources(annotation, path_source_pairs) {
  const values = [];
  const sources = [];
  for (const [path, source] of path_source_pairs) {
...
function _get_disease_descriptions(annotation) {
  const { values, sources } = _get_from_sources(annotation, [
    [_CONSTANTS.DISEASE.DESCRIPTION.MONDO, SOURCES.MONDO],
    [_CONSTANTS.DISEASE.DESCRIPTION.DISEASE_ONTOLOGY, SOURCES.DISEASE_ONTOLOGY]
  ]);
...
function _get_disease_mesh_curies(annotation) {
  const { values, sources } = _get_from_sources(annotation, [
    [_CONSTANTS.DISEASE.MESH.MONDO, SOURCES.MONDO],
    [_CONSTANTS.DISEASE.MESH.DISEASE_ONTOLOGY, SOURCES.DISEASE_ONTOLOGY]
  ]);
  if (cmn.is_array_empty(values)) return _empty_section();
...
function _get_chemical_names(annotation) {
  ...
  if (!cmn.is_array_empty(pharm_trade_names)) {
    sources.push(SOURCES.PHARMGKB);
  }
...
function _get_chemical_descriptions(annotation) {
  const { values, sources } = _get_from_sources(annotation, [
    [_CONSTANTS.CHEMICAL.DESCRIPTION.NCIT, SOURCES.NCIT],
    [_CONSTANTS.CHEMICAL.DESCRIPTION.CHEBI, SOURCES.CHEBI]
  ]);
  if (cmn.is_array_empty(values)) return _empty_section();
...
function _get_chebi_roles(annotation) {
  ...
  return make_section(roles, [SOURCES.CHEBI]);
}
...
function _get_drug_indications(annotation) {
  ...
  return make_section(indications, [SOURCES.CHEMBL]);
}
...
function _get_clinical_trials(annotation) {
  const clinical_trials = _get(annotation, _CONSTANTS.CHEMICAL.CT);
  if (cmn.is_missing(clinical_trials)) return _empty_section();
  return make_section(clinical_trials.map(record => record.id), [SOURCES.CLINICAL_TRIALS]);
}
...
function _get_gene_description(annotation) {
  ...
  return make_section(cmn.coerce_array(gene_summary), [SOURCES.NCBI_GENE]);
}
...
function _get_gene_name(annotation) {
  ...
  return make_section(name, [SOURCES.NCBI_GENE]);
}
...
function _get_gene_species(annotation) {
  ...
  return make_section(species, [SOURCES.NCBI_TAXONOMY]);
}
...
function _get_gene_tdl(annotation) {
  ...
  return make_section(cmn.coerce_array(tdl), [SOURCES.PHAROS]);
}
