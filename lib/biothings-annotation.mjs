...
function _get_disease_synonyms(annotation) {
  ...
  return make_section(synonyms, contributions.flatMap(contribution => contribution.sources));
}
...
function _get_mondo_labels(annotation) {
  ...
  return {
    labels: labels,
    sources: [make_source(SOURCES.MONDO, _get(annotation, _CONSTANTS.DISEASE.ID.MONDO))]
  };
}
...
function _get_disease_ontology_labels(annotation) {
  ...
  return {
    labels: labels,
    sources: [make_source(SOURCES.DISEASE_ONTOLOGY, doid)]
  };
}
