...
function inject_curies(node) {
  const curies = cmn.distinct_array(node.curies ?? []);
  if (cmn.is_array_empty(curies)) return node;
  for (const type of Object.values(_CONSTANTS.TYPE)) {
    if (!_is_type(node, type)) continue;
    const annotation = _get(node, type);
    annotation.curies = make_section(curies.map(make_curie_entry), annotation.curies?.metadata?.sources ?? null);
  }
  return node;
}
...
function make_curie_entry(curie) {
  return { curie: curie, url: bl.curie_to_url(curie) || null };
}
...
function make_source(source, id = null) {
  ...
  return { id: source.id, name: source.name, url: url };
}
...
function _get_disease_mesh_curies(annotation) {
  ...
  if (cmn.is_array_empty(values)) return _empty_section();
  return make_section(values.map(curie => make_curie_entry(`MESH:${curie}`)), sources);
}
...
const SOURCES = Object.freeze({
  ...
  DRUG_APPROVALS: Object.freeze({
    id: "drug_approvals",
    name: "Multiomics Drug Approvals KP",
    ...
    OTC: Object.freeze({
      ...
      MAP_ID_STRING: (otc_code) => {
        switch(otc_code) {
          ...
          case 1: return "Prescription Only";
