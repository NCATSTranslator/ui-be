...
function make_rule_collect_chemical_annotations() {
  return _make_annotation_rule([
    ...
    [_get_otc_status, (target, otc_status) => {
      if (cmn.is_missing(otc_status)) {
        target.otc_status = null;
      } else {
        target.otc_status = {
          code: otc_status,
  ...
  return _make_annotation_rule([
    ...
    [_get_tdl, (target, tdl) => {
      if (cmn.is_missing(tdl)) {
        ...
        target.tdl = cmn.coerce_array(tdl);
      }
      return target;
    }]
...
function make_rule_collect_disease_annotations() {
  return _make_annotation_rule([
    [_get_disease_descriptions, (target, descriptions) => {
      target.descriptions = descriptions;
      return target;
    }],
...
function _get_disease_descriptions(annotation) {
  const paths = [
    _CONSTANTS.DISEASE.DESCRIPTION.MONDO,
    _CONSTANTS.DISEASE.DESCRIPTION.DISEASE_ONTOLOGY
  ];
  const descriptions = _get_many(annotation, paths);
  if (cmn.is_array_empty(descriptions)) return null;
  return descriptions.map(description => description.split("[")[0].replaceAll("\"", ""));
}
...
function _get_otc_status(annotation) {
  return _get(annotation, _CONSTANTS.CHEMICAL.OTC.ID);
}

function _get_clinical_trials(annotation) {
  const clinical_trials = _get(annotation, _CONSTANTS.CHEMICAL.CT);
  if (cmn.is_missing(clinical_trials)) return null;
  return clinical_trials.map(record => record.id);
}

function _get_gene_description(annotation) {
  const gene_summary = _get(annotation, _CONSTANTS.GENE.SUMMARY);
  if (cmn.is_missing(gene_summary)) return null;
  return cmn.coerce_array(gene_summary);
}

function _get_gene_name(annotation) {
  return _get(annotation, _CONSTANTS.GENE.NAME);
}
