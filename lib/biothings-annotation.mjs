...
function _get_chemical_names(annotation) {
  ...
  if (!cmn.is_array_empty(pharm_trade_names)) {
    sources.push(make_source(SOURCES.PHARMGKB));
  }
  ...
  if (!cmn.is_array(ndc_entries)) {
    sources.push(make_source(SOURCES.NDC));
...
function _get_chebi_roles(annotation) {
  ...
  return make_section(roles, [make_source(SOURCES.CHEBI, _get(annotation, _CONSTANTS.CHEMICAL.CHEBI_ID))]);
}
...
function _get_drug_indications(annotation) {
  ...
  for (const entry of approval_entries) {
    ...
    const indication = _clinical_approval_indication(_get(entry, _CONSTANTS.CHEMICAL.CLINICAL_APPROVAL.DISEASE));
    if (cmn.is_missing(indication)) continue;
    const existing = indications.get(indication.name);
    if (existing === undefined) {
      indications.set(indication.name, { name: indication.name, ids: [indication.id] });
    } else if (!existing.ids.includes(indication.id)) {
      existing.ids.push(indication.id);
    }
  }
  if (indications.size === 0) return _empty_section();
  return make_section([...indications.values()], [make_source(SOURCES.DRUG_APPROVALS)]);
}

function _clinical_approval_indication(disease) {
...
function _get_clinical_trials(annotation) {
  ...
  return make_section(clinical_trials.map(record => record.id), [make_source(SOURCES.CLINICAL_TRIALS)]);
}
...
function _get_gene_name(annotation) {
  ...
  return make_section(name, [make_source(SOURCES.NCBI_GENE, _get(annotation, _CONSTANTS.GENE.ID))]);
}
...
const SOURCES = Object.freeze({
  ...
  DRUG_APPROVALS: Object.freeze({
    name: "Drug Approvals Knowledge Provider",
  ...
  PHAROS: Object.freeze({
    ...
    link: (id) => `https://pharos.nih.gov/targets/${id}`
  })
});
