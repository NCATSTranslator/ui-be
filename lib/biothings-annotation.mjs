...
export function getCuries(annotation) {
  const fallbackHandler = noHandler([]);
  return parseAnnotation(
    annotation,
...
function parseAnnotation(annotation, diseaseHandler, chemicalHandler, geneHandler, defaultHandler = noHandler()) {
...
function noHandler(fallback = null) {
  return () => fallback;
}
