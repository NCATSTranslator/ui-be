...
class QGraph {
  ...
  get template() {
    ...
    } else if (this.qNodeCount() === 3 && this.qEdgeCount() === 3) {
...
export function getQueryGraph(trapiMessage) {
  const qg = cmn.jsonGet(trapiMessage, CONSTANTS.QGRAPH.KEY, false);
  if (!qg) {
...
export function getResults(trapiMessage) {
  const results = cmn.jsonGet(trapiMessage, CONSTANTS.RESULTS.KEY, false);
  return results;
}

export function getAuxGraphs(trapiMessage) {
  const auxGraphs = cmn.jsonGet(trapiMessage, CONSTANTS.AGRAPH.KEY, false);
  return auxGraphs;
}
...
export function getKgraph(trapiMessage) {
  return cmn.jsonGet(trapiMessage, CONSTANTS.GRAPH.KEY);
}

export function getCuries(kgraph) {
  return cmn.jsonGetKeys((cmn.jsonGet(kgraph, CONSTANTS.GRAPH.NODES, {})));
}
