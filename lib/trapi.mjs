...
class QEdge {
  constructor(sub, obj, predicate, /* TODO: edgeMode, */ constraints) {
    this.subject = sub;
    this.object = obj;
  ...
  static fromTrapi(trapiQEdge, qNodes) {
    ...
    return new QEdge(qNodes[sub], qNodes[obj], predicate, constraints);
  }
...
export class AttributeIterator {
  ...
  findOne(searchIds, sentinel = null) {
    while (this.hasNext()) {
      ...
      if (searchIds.includes(getAttrId(attribute))) {
        const v = getAttrVal(attribute);
        if (v) return v;
      }
    }
  ...
  findAll(searchIds, sentinel = null) {
    const result = [];
    while (this.hasNext()) {
      const val = this.findOne(searchIds, sentinel);
      if (val === sentinel) break;
      result.push(...cmn.coerceArray(val));
    }
...
export function getAttrId(attribute) {
  return cmn.jsonGet(attribute, CONSTANTS.GRAPH.ATTRIBUTES.ID);
}

export function getAttrVal(attribute) {
 ...
 */
export function getPrimarySource(graphElem) {
  const sources = cmn.jsonGet(graphElem, CONSTANTS.GRAPH.SOURCES.KEY, []);
...
export function getSupGraphs(kedge) {
  const attributes = getAttrs(kedge);
  const attrIter = new AttributeIterator(attributes);
  const supGraphs = attrIter.findOne([bl.tagBiolink('support_graphs')]);
  if (supGraphs === null) return [];
  return supGraphs;
}
...
export function messageToEndpoints(message) {
  ...
  const startIsObj = cmn.jsonGetFromKpath(qg, ['nodes', SUBJECT_KEY, 'ids'], false);
  if (startIsObj) {
