...
export function makeMetadataObject(qid, agents, queryType) {
...
export function queryToCreativeQuery(query) {
  ...
  function genPathfinderQgraph(subject, object, constraint) {
    function makeQNode(curie, category) {
      const node = {};
      if (curie) {
        node['ids'] = [curie];
      }
    ...
    function makeQEdge(subjectId, objectId) {
      const edge = {};
      edge['subject'] = subjectId;
      edge['object'] = objectId;
      edge['predicates'] = [bl.tagBiolink('related_to')];
    ...
    const interKey = 'un';
    const nodes = {};
    nodes[subjectKey] = makeQNode(subject.id, subject.category);
    nodes[objectKey] = makeQNode(object.id, object.category);
    if (!constraint) {
      constraint = bl.tagBiolink('NamedThing');
    }
    nodes[interKey] = makeQNode(false, constraint);
    const edges = {};
    edges['e0'] = makeQEdge(subjectKey, interKey);
    edges['e1'] = makeQEdge(interKey, objectKey);
    edges['e2'] = makeQEdge(subjectKey, objectKey);
    return {
      'nodes': nodes,
      'edges': edges
    };
  ...
  switch (queryType) {
    ...
    case 'chemical':
      ...
      break;
    case 'pathfinder':
      const subject = cmn.jsonGet(query, 'subject');
      const object = cmn.jsonGet(query, 'object');
      const constraint = cmn.jsonGet(query, 'constraint', false);
      qg = genPathfinderQgraph(subject, object, constraint);
      break;
    default:
...
function isPathfinderQuery(queryType) {
  return queryType === QUERY_TYPE.PATHFINDER;
}
...
function answerToQueryType(answer) {
  ...
  if (isPathfinderQGraph(qg)) {
    return QUERY_TYPE.PATHFINDER;
  }
...
function isExternalTag(tag) {
  ...
  const validFamilies = ['cc', 'di', 'pc', 'pt', 'role', 'ara', 'otc'];
...
function determineAnswerTag(type, answerTags, queryType) {
  ...
  const fdaTags = Object.keys(answerTags).filter((tag) => { return tag.startsWith('r/fda'); });
  ...
  if (highestFdaApproval === 0) return ['r/cc/other', 'Other'];

  if (isDrug(type, highestFdaApproval)) return ['r/cc/drug', 'Drug'];

  if (isClinicalPhase(highestFdaApproval)) return [`r/cc/phase${highestFdaApproval}`, `Phase ${highestFdaApproval} Drug`];

  return [`r/cc/other`, `Other`];
}
...
function analysisToRgraph(analysis, kgraph, auxGraphs) {
  ...
  while (!cmn.isArrayEmpty(unprocessedEdgeBindings) || !cmn.isArrayEmpty(unprocessedSupportGraphs)) {
    while (!cmn.isArrayEmpty(unprocessedEdgeBindings)) {
      ...
      // TODO: replace this check with a more generic version that will give full control based on edge
      if (!cmn.isArrayEmpty(edgeSupportGraphs) && bl.sanitizeBiolinkItem(kedgePredicate(kedge)) === 'related to') {
        // We want to effectively embed any supporting graph into the graph the edge appears in
        ...
        edgeBindingData[eb].partOf = []; // This states that the edge is no longer a part of any graph, effectively removing it
        edgeSupportGraphs.forEach((sg) => {
          if (!supportGraphs.has(sg)) {
            unprocessedSupportGraphs.push([sg, gidProxies]);
          }
        });
      } else {
        edgeSupportGraphs.forEach((sg) => {
          if (!supportGraphs.has(sg)) {
            unprocessedSupportGraphs.push([sg, [sg]]);
          }
        });
      }
    };
    ...
    while (!cmn.isArrayEmpty(unprocessedSupportGraphs)) {
      const [gid, gidProxies] = unprocessedSupportGraphs.pop();
...
function isPathfinderQGraph(qg) {
  const nodeBindings = cmn.jsonGet(qg, 'nodes', false);
  const edgeBindings = cmn.jsonGet(qg, 'edges', false);
  return !!nodeBindings &&
         !!edgeBindings &&
         Object.keys(nodeBindings).length === 3 &&
         Object.keys(edgeBindings).length === 3;
}

function getPathDirection(qgraph) {
  if (isPathfinderQGraph(qgraph)) {
...
function isRootPath(pid, paths, edges) {
  ...
  for (let i = 1; i < path.length; i+=2) {
    isRoot = isRoot && edges[path[i]].is_root;
  }
  return isRoot;
}
...
function genMetaPath(path, nodes) {
  ...
  for (let i = 0; i < path.length; i+=2) {
    ...
    metaPath.push(bl.sanitizeBiolinkItem(node.types[0]));
  }
...
async function summaryFragmentsToSummary(qid, condensedSummaries, kgraph, queryType, agentToName, errors) {
  function fragmentPathsToResultsAndPaths(fragmentPaths, nodes, queryType) {
    // TODO: use objects instead of arrays?
    let results = [];
    let paths = [];
    fragmentPaths.forEach((path) => {
      const pathKey = pathToKey(path);
      let resultKey = path[0];
      if (isPathfinderQuery(queryType)) {
        resultKey = pathToKey(genMetaPath(path, nodes));
      }
      results.push(cmn.makePair(resultKey, pathKey, 'start', 'pathKey'));
  ...
  function resultsToResultsAndTags(results, paths, nodes, edges, scores, errors, queryType) {
    ...
    function markPathAsIndirect(paths, edges, pid) {
      function helper(paths, edges, pid, seen) {
        seen.add(pid);
        const tag = 'p/pt/inf';
        const path = paths[pid];
        const subgraph = path.subgraph;
        for (let i = 1; i < subgraph.length; i+=2) {
          const edge = edges[subgraph[i]];
          if (!cmn.isArrayEmpty(edge.support)) {
            for (let spid of edge.support) {
              if (!seen.has(spid)) {
                markPathAsIndirect(paths, edges, spid);
              }
            }
          }
        }
        path.tags[tag] = makeTagDescription('Indirect');
      }

      helper(paths, edges, pid, new Set());
    }

    function genName(nodes, subgraph, queryType) {
      function getNodeName(nodes, nid) {
        const names = cmn.jsonGetFromKpath(nodes, [nid, 'names']);
        return (cmn.isArrayEmpty(names)) ? nid : names[0];
      }
      if (isPathfinderQuery(queryType)) {
        const metaPath = genMetaPath(subgraph, nodes);
        metaPath[0] = getNodeName(nodes, subgraph[0]);
        metaPath[metaPath.length-1] = getNodeName(nodes, subgraph[subgraph.length-1]);
        return metaPath.join('/');
      }
      return getNodeName(nodes, subgraph[0]);
    }

    function genId(nodes, subgraph, queryType) {
      if (isPathfinderQuery(queryType)) {
        const metaPath = genMetaPath(subgraph, nodes);
        return pathToKey(metaPath);
      }
      const start = subgraph[0];
      const end = subgraph[subgraph.length-1];
      return pathToKey([start, end]);
    }
    ...
    for (const result of results) {
      ...
      const name = genName(nodes, subgraph, queryType);
      const start = subgraph[0];
      const end = subgraph[subgraph.length-1];
      const tags = {};
      pids.forEach((pid) => {
      ...
      // Generate inferred/lookup tags for results and paths
      rootPids.forEach((pid) => {
        const subgraph = getPathFromPid(paths, pid);
        const edge = edges[subgraph[1]];
        const pathIsIndirect = edge.support.length > 0;
        if (pathIsIndirect) {
          const tag = 'p/pt/inf';
          markPathAsIndirect(paths, edges, pid);
          ...
          paths[pid].tags[tag] = directTag;
          tags[tag] = null;
        }
      });

      expandedResults.push({
        'id': genId(nodes, subgraph, queryType),
        'subject': start,
        'drug_name': name,
  ...
  condensedSummaries.forEach((cs) => { const agents = condensedSummaryAgents(cs);
    const [newResults, newPaths] = fragmentPathsToResultsAndPaths(condensedSummaryPaths(cs), nodes, queryType);
  ...
  const metadataObject = makeMetadataObject(qid,
    cmn.distinctArray(condensedSummaries.map((cs) => { return cs.agents; }).flat()),
    queryType);
  try {
    // Node annotation
    const nodeRules = makeSummarizeRules(
      [
        ...
        renameAndTransformAttribute(
          ...
          ['species'],
          (annotations) => {
            const species = bta.getSpecies(annotations);
            return species;
          }
        )
      ]
    ...
    const resultNodeRules = makeSummarizeRules(
      [
        ...
        tagAttribute(
          'biothings_annotations',
          (annotations) => {
            const otc = bta.getOtc(annotations);
            if (otc === null) {
              return [];
            }
    ...
    Object.keys(paths).forEach((pk) => {
      ...
      if (isChemicalDiseaseQuery(queryType)) {
        ...
        if (start.indications !== undefined) {
          ...
          if (indicatedFor) {
            start.tags['r/di/ind'] = makeTagDescription('In a clinical trial for indicated disease');
          } else {
            start.tags['r/di/not'] = makeTagDescription('Not in a clinical trial for indicated disease');
          }
        }
      ...
      // Add tags for paths by processing nodes (skip the last node)
      const tags = {};
      for (let i = 0; i < path.subgraph.length-1; ++i) {
        if (isNodeIndex(i)) {
          ...
          if (node !== undefined) { // Remove me when result graphs are fixed
            ...
            if (i === 0) {
              ...
              Object.keys(node.tags)
                .filter((tag) => { return isResultTag(tag) && isExternalTag(tag); })
                .forEach((tagID) => { tags[tagID] = node.tags[tagID]; });
              ...
              // Merge all node tags into the path for intermediate nodes
              Object.keys(node.tags)
                .filter((tag) => { return !isResultTag(tag) && isExternalTag(tag); })
                .forEach((tagID) => { tags[ID] = node.tags[ID]; });

              // Generate tags based on the node category
              tags[`p/pc/${type}`] = makeTagDescription(type);
            }
          }
        }
      }
      ...
      aras.forEach((ara) => {
        tags[`r/ara/${ara}`] = makeTagDescription(agentToName(ara));
      });
      ...
      tags[`p/pt/${pathLength}`] = makeTagDescription(`${pathLength} ${tagDescription}`);
      path.tags = tags;
    });
...
export class UnknownQueryError extends Error {
  constructor(queryType) {
    super(`Query type unrecognized: ${queryType}`);
  }
}
