  ...
  GRAPH: {
    ...
    EDGE: {
      ...
      QUALIFIER: {
        ...
        VALUE: 'qualifier_value',
        OBJECT: {
          DIRECTION: bl.tagBiolink('object_direction_qualifier')
        }
      }
    },
    ATTRIBUTES: {
...
class QNode {
  constructor(binding, category, curies) {
    ...
    if (cmn.isArray(curies) && !cmn.isArrayEmpty(curies)) {
      this.ids = curies;
    }
  ...
  static from_trapi(binding, trapiQNode) {
    const categories = cmn.jsonGet(trapiQNode, 'categories');
    let category = null;
    if (cmn.isArray(categories) && categories.length > 0) {
      category = categories[0];
    }
    const curies = cmn.jsonGet(trapiQNode, 'ids', null);
...
class QEdge {
  constructor(sub, obj, pred, /* TODO: edgeMode, */ constraints) {
    ...
    if (cmn.isArray(constraints) && !cmn.isArrayEmpty(constraints)) {
  ...
  static from_trapi(trapiQEdge, qNodes) {
    ...
    const predicates = cmn.jsonGet(trapiQEdge, 'predicates');
    let pred = null;
    if (cmn.isArray(predicates) && !cmn.isArrayEmpty(predicates)) {
      pred = predicates[0];
    }
    const constraints = cmn.jsonGet(trapiQEdge, 'qualifier_constraints', []).map(constraint => {
...
class QEdgeQualifierSet {
  ...
  get(key) {
    for (const qualifier of this[CONSTANTS.QUALIFIERS.SET]) {
      if (qualifier[CONSTANTS.QUALIFIERS.ID] === key) {
        return qualifier[CONSTANTS.QUALIFIERS.VALUE];
      }
    }
    return null;
  }
...
class QPath {
  ...
  static from_trapi(binding, trapi_qpath) {
    ...
    if (constraints) {
      constraint = cmn.jsonGet(constraints[0], 'intermediate_categories', null);
    }
...
function makeQEdgeDirectionQualifier(direction) {
  return new QEdgeQualifier(CONSTANTS.GRAPH.EDGE.QUALIFIER.OBJECT.DIRECTION, direction);
}
...
export function query_graph_to_client_request(query_graph) {
  const qg = QGraph.from_trapi(query_graph);
  const query_type = qg.template;
  if (isChemicalDiseaseQuery(query_type)) {
    return {
      pid: null,
      type: "drug",
      curie: qg.qNodes[OBJECT_KEY].ids[0],
      direction: null,
      node_one_label: null
    }
  } else if (isGeneChemicalQuery(query_type)) {
    return _make_gene_chemical_request(qg, "gene", SUBJECT_KEY);
  } else if (isChemicalGeneQuery(query_type)) {
    return _make_gene_chemical_request(qg, "chemical", OBJECT_KEY);
  } else if (isPathfinderQuery(query_type)) {
    const subject = qg.qNodes[SUBJECT_KEY];
    const object = qg.qNodes[OBJECT_KEY];
    const path = Object.values(qg.qConnections)[0];
    return {
      pid: null,
      type: "pathfinder",
      subject: {
  ...
  function _make_gene_chemical_request(qg, type, key) {
    const qEdge = Object.values(qg.qConnections)[0];
    return {
      pid: null,
      type: type,
      curie: qg.qNodes[key].ids[0],
      direction: qEdge.qualifiers.get(CONSTANTS.GRAPH.EDGE.QUALIFIER.OBJECT.DIRECTION),
      node_one_label: null
    }
  }
}
...
export function getPrimarySrc(graphElem) {
  ...
  for (let src of srcs) {
    ...
    const records = cmn.jsonGet(src, CONSTANTS.GRAPH.SOURCES.URLS, null);
    if (!role || !id) continue;
...
function _isChemicalEntity(category) {
  const aliases = [bl.tagBiolink('ChemicalEntity'), bl.tagBiolink('SmallMolecule'), bl.tagBiolink('Drug')];
  return aliases.includes(category);
}

function _isGene(category) {
  const aliases = [bl.tagBiolink('Gene'), bl.tagBiolink('Protein')];
  return aliases.includes(category);
}
