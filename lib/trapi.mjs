...
export function queryToCreativeQuery(query) {
  ...
  function genPathfinderQgraph(subject, object) {
    function makeQNode(id, curie, category) {
      const node = {};
      if (id && curie) {
        node[id] = [curie];
      }
    ...
    const interKey = 'un';
    const nodes = {};
    nodes[subjectKey] = makeQNode(subjectKey, subject[0], subject[1]);
    nodes[objectKey] = makeQNode(objectKey, object[0], object[1]);
    nodes[interKey] = makeQNode(false, false, 'NamedThing');
    const edges = {};
