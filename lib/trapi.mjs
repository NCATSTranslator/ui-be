...
export function clientReqToTrapiQuery(clientReq) {
  ...
  function makePathfinderQgraph(subject, object, constraint) {
    ...
    const qNodes = {};
    qNodes[subNode.binding] = subNode.toTrapi();
    qNodes[objNode.binding] = objNode.toTrapi();
    qNodes[interNode.binding] = interNode.toTrapi();
    const qEdges = {};
    const predicate = 'related_to';
    qEdges['e0'] = (new QEdge(subNode, interNode, predicate)).toTrapi();
    qEdges['e1'] = (new QEdge(interNode, objNode, predicate)).toTrapi();
    qEdges['e2'] = (new QEdge(subNode, objNode, predicate)).toTrapi();
    return new QGraph(qNodes, qEdges);
  }
