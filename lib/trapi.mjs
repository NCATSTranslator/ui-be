...
export {
  ...
  make_rule_transform_and_rename_attribute,
...
function get_node_binding(result, key) {
...
function get_edge_type(kedge) {
  ...
  if (cmn.isArrayEmpty(support_graphs)) return CONSTANTS.GRAPH.EDGE.TYPE.DIRECT;
...
function message_to_endpoints(message) {
  if (SUBJECT_KEY === null || OBJECT_KEY === null) throw new cmn.DeveloperError('trapi.mjs', 'message_to_endpoints', `SUBJECT_KEY (${SUBJECT_KEY}) and OBJECT_KEY (${OBJECT_KEY}) must be configured`);
  const qgraph = _QGraph.from_trapi(_get_query_graph(message));
  ...
  return _make_property_rule({
    ...
    update: (target, property) => {
      ...
      if (!cmn.is_missing(property)) {
        current_value.push(...cmn.coerceArray(property));
      }
      return target;
    },
    default_value: []
...
function make_rule_aggregate_property({source_key, target_key}) {
  return make_rule_transform_and_aggregate_property({
    ...
    transform: (obj) => { return cmn.jsonGet(obj, key); }
  });
}
 ...
 * @returns {function} - The extraction rule.
 */
function make_rule_transform_and_rename_attribute({attr_id, target_key, transform}) {
  const default_value = null;
  return _make_attribute_rule({
    attr_ids: attr_id,
    transform: (attrs, attr_id) => {
      const attr_itr = new _AttributeIterator(attrs);
      const value = attr_itr.find_one_val(attr_id);
 ...
 * @returns {function} - The extraction rule.
 */
function make_rule_map_attributes({attr_ids, target_key, transform}) {
  return _make_attribute_rule({
    attr_ids: attr_ids,
    transform: (attrs, attr_ids, context) => {
      const attr_itr = new _AttributeIterator(attrs);
      const vals = attr_itr.find_all_val(attr_ids);
      return result.map(val => transform(val, context)).flat();
    },
    update: (target, attrs) => {
 ...
 * @returns {function} - The extraction rule.
 */
function make_rule_collect_attributes(kwargs) {
...
class _AttributeIterator {
  ...
  find_one(search_ids, sentinel = null) {
    search_ids = cmn.coerceArray(search_ids);
    while (this.has_next()) {
  ...
  find_one_val(search_ids, sentinel = null) {
    search_ids = cmn.coerceArray(search_ids);
  ...
  find_all_val(search_ids, sentinel = null) {
    search_ids = cmn.coerceArray(search_ids);
    const result = [];
    while (this.has_next()) {
      const attr = this.find_one(search_ids, sentinel);
      if (attr === sentinel) break;
      const attr_val = get_attr_val(attr);
      if (attr_val !== null) {
        result.push(...cmn.coerceArray(attr_val));
      }
    }
    return result;
  }
 ...
 * @returns {function} - The extraction rule.
 */
function _make_property_rule({source_key, transform, update, default_value}) {
...
function _make_attribute_rule({attr_ids, transform, update, default_value} ) {
  return (source, context) => {
    return _make_rule({
      source: source[CONSTANTS.GRAPH.ATTRIBUTES.KEY] ?? [],
      key: attr_ids,
      transform: transform,
      update: update,
      context: context,
...
function _make_rule({source, key, transform, update, context, default_value}) {
  return (target) => {
    try {
...
function _get_graph_element_attr_val(graph_element, id, default_value = null) {
  const attrs = get_attrs(graph_element);
  const attr_itr = new _AttributeIterator(attrs);
  const attr_val = attr_itr.find_one_val(id);
  if (attr_val === null) return default_value;
  return attr_val;
}

function _has_no_attrs(attrs) {
  return cmn.is_missing(attrs) || cmn.isArrayEmpty(attrs);
}
...
function _make_trapi_message(qgraph, kgraph) {
  if (!(qgraph instanceof _QGraph)) {
    throw new cmn.DeveloperError('lib/trapi.mjs', '_make_trapi_message', `qgraph is not an instance of _QGraph\n  Got: ${qgraph}`);
  }
  const message = {
...
class _Query {
  constructor(client_request) {
    if (!cmn.isObject(client_request)) {
