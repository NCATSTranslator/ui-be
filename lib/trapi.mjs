...
class QEdge {
  ...
  static fromTrapi(trapiQEdge, qNodes) {
    ...
    return new QEdge(qNodes[subject], qNodes[object], predicate, constraints);
  }
...
class QGraph {
  ...
  get template() {
    ...
    const startIsObject = this.qNodes[SUBJECT_KEY].ids
    if (startIsObject) {
...
export function getQueryGraph(trapiMessage) {
  return cmn.jsonGetFromKpath(trapiMessage, [CONSTANTS.ROOT, CONSTANTS.QGRAPH.KEY]);
}

export function getKnowledgeGraph(trapiMessage) {
...
export function getKnowledgeEdge(edgeBinding, kgraph) {
  return getKnowledgeGraphElement(edgeBinding, CONSTANTS.GRAPH.EDGES, kgraph);
}

export function getKnowledgeNode(nodeBinding, kgraph) {
  return getKnowledgeGraphElement(nodeBinding, CONSTANTS.GRAPH.NODES, kgraph);
}
...
export function getAttributes(graphElement) {
  return cmn.jsonGet(graphElement, CONSTANTS.GRAPH.ATTRIBUTES.KEY, []);
}
...
export function getSupportGraphs(kedge) {
  const attributes = getAttributes(kedge);
  const attrIter = new AttributeIterator(attributes);
  return attrIter.findOne([bl.tagBiolink('support_graphs')]);
}

export function getQualifiers(kedge) {
  const trapiQualifiers = cmn.jsonGet(kedge, 'qualifiers', []);
  if (!cmn.isArray(trapiQualifiers)) {
    throw new InvalidQualifiersError(kedge);
  }

  return trapiQualifiers;
}
...
export function getQualifierValue(qualifier) {
...
function getKnowledgeGraphElement(binding, type, kgraph) {
