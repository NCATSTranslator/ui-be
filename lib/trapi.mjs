...
export function clientReqToTrapiQuery(clientReq) {
  ...
  switch (query.template) {
    case CONSTANTS.QGRAPH.TEMPLATE.CHEMICAL_DISEASE:
      return makeTrapiMessage(diseaseToTrapiQgraph(query.curie), null);
    case CONSTANTS.QGRAPH.TEMPLATE.GENE_CHEMICAL:
      return makeTrapiMessage(chemicalToTrapiQgraph(query.curie, query.direction), null);
    case CONSTANTS.QGRAPH.TEMPLATE.CHEMICAL_GENE:
      return makeTrapiMessage(geneToTrapiQgraph(query.curie, query.direction), null);
    default:
      throw new RangeError(`Expected query type to be one of [drug, gene, chemical], got: ${queryType}`);
  }
}
 ...
 *
 * @param {object} answer - The answer to determine the query template from.
 *
 * @returns {number} - The query template.
 */
