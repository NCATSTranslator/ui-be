...
// TODO: make these a configuration
const subjectKey = 'sn';
const objectKey = 'on';
...
export function answersToSummary (qid, answers, maxHops, nodeAnnotationClient) {
  ...
  const queryType = answersToQueryType(answers);
...
function answerToQueryType(answer) {
  ...
  const [subjectKey, objectKey] = qgraphToPathEndpoints(qg);
...
class SummaryEdge {
  ...
  merge(summaryEdge) {
    ...
    this.is_root = this.is_root|| summaryEdge.isRootPath;
    return this
  }
...
/*
 * Determine which keys (globally set) correspond to the start and end of a path given a query graph.
 *
 * @param {object} qg - The query graph to determine the start and end keys from.
 *
 * @returns {string[]} - The keys corresponding to the start and end of a path.
 */
function qgraphToPathEndpoints(qg) {
  const startIsObject = cmn.jsonGetFromKpath(qg, ['nodes', subjectKey, 'ids'], false);
  if (startIsObject) {
...
function answersToSummaryFragments(answers, nodeRules, edgeRules, maxHops) {
  ...
  answers.forEach((answer) => {
    ...
    const [startKey, endKey] = qgraphToPathEndpoints(getAnswerQueryGraph(answer));
...
async function summaryFragmentsToSummary(qid, condensedSummaries, queryType, nodeAnnotationClient, errors) {
  ...
  function extendSummaryEdges(edges, edgeUpdates, agents) {
    extendSummaryObj(edges, edgeUpdates, agents, () => new SummaryEdge());
  }
  ...
  condensedSummaries.forEach((cs) => {
    ...
    Object.keys(summaryEdges.base).forEach((k) => {
      ...
      edge.merge(summaryEdges.base[k]);
    });
  ...
  const annotationPromise = nodeAnnotationClient.annotateGraph(createKGFromNodeIds(Object.keys(nodes)));
