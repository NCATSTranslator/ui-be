 ...
 */
function client_request_to_trapi_query(clientReq) {
  const query = new _Query(clientReq);
  if (isChemicalDiseaseQuery(query.template)) {
    return makeTrapiMessage(__disease_to_qgraph(query.curie));
  } else if (isGeneChemicalQuery(query.template)) {
    return makeTrapiMessage(__chemical_to_qgraph(query.curie, query.direction));
  } else if (isChemicalGeneQuery(query.template)) {
    return makeTrapiMessage(__gene_to_qgraph(query.curie, query.direction));
    ...
    return makeTrapiMessage(__make_pathfinder_query_graph(subject, object, query.constraint));
  }
  throw new RangeError(`Expected query type to be one of [drug, gene, chemical, pathfinder], got: ${queryType}`);
  ...
  function __make_smart_query_graph(subject, object, qedge) {
    ...
    return (new _QGraph(qnodes, qedges)).to_trapi();
  }
  ...
  function __make_pathfinder_query_graph(subject, object, constraint) {
    ...
    const qpath = new _QPath(_QPath.gen_binding(), subject_node.binding, object_node.binding, constraint);
    const qpaths = {};
    qpaths[qpath.binding] = qpath;
    const trapi_qgraph = (new _QGraph(qnodes, qpaths, 'pathfinder')).to_trapi();
    return trapi_qgraph;
  }
...
export function messageToQueryTemplate(message) {
  const qg = _QGraph.from_trapi(_get_query_graph(message));
  return qg.template;
}
...
function _get_query_graph(trapiMessage) {
  const qg = cmn.jsonGetFromKpath(trapiMessage, [CONSTANTS.ROOT, CONSTANTS.QGRAPH.KEY], false);
  if (!qg) throw new MissingQueryGraphError(trapiMessage);
  return qg;
}
...
class _QEdge {
  ...
  static from_trapi(trapiQEdge, qNodes) {
    ...
    return new _QEdge(qNodes[sub], qNodes[obj], pred, constraints);
  }
