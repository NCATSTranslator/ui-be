...
export function isClinicalTrial(id) {
  return typeof(id) === 'string' && id.startsWith('NCT');
}
...
export function genKnowledgeSrcUrl(edge, inforesId, nodes) {
  const generator = _SPECIFIC_KNOWLEDGE_SRC_MAPPINGS[inforesId];
  if (!generator) return false;
  return generator(edge, nodes);
}

const _SPECIFIC_KNOWLEDGE_SRC_MAPPINGS = Object.freeze({
  'infores:ctd': _genCTDUrl
});

function _genCTDUrl(edge) {
