...
function client_request_to_trapi_query(client_request) {
  ...
  } else if (is_lookup_query(query.type)) {
    if (cmn.is_missing(query.subject) || cmn.is_missing(query.subject.id)) {
      throw new TypeError(`Expected lookup query subject to have an id, got: ${JSON.stringify(query.subject)}`);
    }
    const subject = new _QNode(SUBJECT_KEY, query.subject.category, [query.subject.id]);
    const object = new _QNode(OBJECT_KEY, query.object.category);
    return _make_trapi_message(__make_lookup_query_graph(subject, object));
  }
  throw new RangeError(`Expected query type to be one of [drug, gene, chemical, pathfinder, lookup], got: ${query.type}`);
  ...
  function __make_lookup_query_graph(subject, object) {
    const predicate = bl.tag_biolink('related_to');
    const qedge = new _QEdge(subject, object, predicate, null, CONSTANTS.QGRAPH.LOOKUP);
    const qnodes = {};
    qnodes[subject.binding] = subject;
    qnodes[object.binding] = object;
    const qedges = {};
  ...
  function __make_gene_chemical_constraints(direction) {
    ...
    qualifier_set.add(_make_qedge_predicate_qualifier(bl.tag_biolink('causes')));
  ...
  function __disease_to_qgraph(disease_curie) {
    ...
    const predicate = bl.tag_biolink('treats');
  ...
  function __gene_to_qgraph(geneCurie, direction) {
    ...
    const predicate = bl.tag_biolink('affects');
...
function get_pk(trapi_message) {
  return cmn.json_get(trapi_message, CONSTANTS.PRIMARY_KEY, false);
}

function get_results(trapi_message) {
  return cmn.json_get_from_kpath(
    trapi_message,
...
function get_auxiliary_graph(gid, graphs) {
  return cmn.json_get(graphs, gid, false);
}

function get_auxiliary_graph_edges(graph) {
  return cmn.json_get(graph, 'edges', []);
}

function get_edge_bindings(analysis) {
  return cmn.json_get(analysis, 'edge_bindings', {})
}

function get_node_bindings(result, key) {
  return cmn.json_get_from_kpath(result, ['node_bindings', key], []);
}

function get_path_bindings(analysis) {
  return cmn.json_get(analysis, 'path_bindings', {});
}

function get_kgraph(trapi_message) {
  return cmn.json_get_from_kpath(trapi_message, [CONSTANTS.ROOT, CONSTANTS.GRAPH.KEY]);
}
...
function get_attrs(graph_element) {
  return cmn.json_get(graph_element, CONSTANTS.GRAPH.ATTRIBUTES.KEY, []);
}

function get_attr_id(attr) {
  return cmn.json_get(attr, CONSTANTS.GRAPH.ATTRIBUTES.ID);
}

function get_attr_val(attr) {
  return cmn.json_get(attr, CONSTANTS.GRAPH.ATTRIBUTES.VALUE);
}
...
function get_primary_source(graph_element) {
  const sources = cmn.json_get(graph_element, CONSTANTS.GRAPH.SOURCES.KEY, []);
  for (let s of sources) {
    const infores = cmn.json_get(s, CONSTANTS.GRAPH.SOURCES.ID, false);
    const role = cmn.json_get(s, CONSTANTS.GRAPH.SOURCES.ROLE, false);
    const records = cmn.json_get(s, CONSTANTS.GRAPH.SOURCES.URLS, null);
    if (role === undefined || infores === undefined) continue;
    if (role === CONSTANTS.GRAPH.SOURCES.PRIMARY) {
      return {
        infores: infores,
        records: cmn.is_missing(records) ? [] : records
      };
    }
  }
...
function get_subject(kedge) {
  return cmn.json_get(kedge, CONSTANTS.GRAPH.EDGE.SUBJECT);
}

function get_object(kedge) {
  return cmn.json_get(kedge, CONSTANTS.GRAPH.EDGE.OBJECT);
}

function get_predicate(kedge) {
  return cmn.json_get(kedge, CONSTANTS.GRAPH.EDGE.PREDICATE);
}

function get_support_graphs(kedge) {
  return _get_graph_element_attr_val(kedge, bl.tag_biolink('support_graphs'), []);
}

function get_qualifiers(kedge) {
  const qualifiers = cmn.json_get(kedge, CONSTANTS.GRAPH.EDGE.QUALIFIER.KEY, []);
...
function get_qualifier_id(qualifier) {
  return cmn.json_get(qualifier, CONSTANTS.GRAPH.EDGE.QUALIFIER.ID, false);
}

function get_qualifier_val(qualifier) {
  return cmn.json_get(qualifier, CONSTANTS.GRAPH.EDGE.QUALIFIER.VALUE);
}
...
function get_knowledge_level(kedge) {
  return _get_graph_element_attr_val(kedge, bl.tag_biolink('knowledge_level'));
}
...
function get_agent_type(kedge) {
  return _get_graph_element_attr_val(kedge, bl.tag_biolink('agent_type'));
}
...
function message_to_endpoints(message) {
  ...
  const qgraph = _QGraph.from_trapi(_get_query_graph(message));
  const query_type = qgraph.get_type();
  if (query_type === CONSTANTS.QGRAPH.TEMPLATE.GENE_CHEMICAL
      || query_type === CONSTANTS.QGRAPH.TEMPLATE.LOOKUP) {
...
function is_lookup_query(query_type) {
  return query_type === CONSTANTS.QGRAPH.TEMPLATE.LOOKUP;
}
...
function _get_kgraph_element(binding, type, kgraph) {
  return cmn.json_get(cmn.json_get(kgraph, type, {}), binding, null);
}
...
function _query_template_to_type(query_type) {
  switch (query_type) {
    ...
    case 'lookup':
      return CONSTANTS.QGRAPH.TEMPLATE.LOOKUP;
    default:
      throw new RangeError(`Expected query type to be one of [drug, gene, chemical, pathfinder, lookup], got: ${query_type}`);
  }
}

function _get_query_graph(trapi_message) {
  const qgraph = cmn.json_get_from_kpath(trapi_message, [CONSTANTS.ROOT, CONSTANTS.QGRAPH.KEY], null);
...
class _Query {
  constructor(client_request) {
    ...
    this.type = _query_template_to_type(cmn.json_get(client_request, 'type', null));
    this.curie = cmn.json_get(client_request, 'curie', null);
    this.direction = cmn.json_get(client_request, 'direction', null);

    // TODO: This is only for pathfinder, we need to fix this
    this.subject = cmn.json_get(client_request, 'subject', null);
    this.object = cmn.json_get(client_request, 'object', null);
    this.constraint = cmn.json_get(client_request, 'constraint', null);
  }
}
...
class _QNode {
  constructor(binding, category, curies) {
    ...
    this.categories = [bl.tag_biolink(category)];
  ...
  static from_trapi(binding, trapi_qnode) {
    const category = cmn.json_get(trapi_qnode, 'categories')[0];
    const curies = cmn.json_get(trapi_qnode, 'ids', []);
    return new _QNode(binding, category, curies);
  }
}

class _QEdge {
  constructor(subject, object, predicate, constraints, query_mode = CONSTANTS.QGRAPH.INFERRED) {
    this.subject = subject;
    this.object = object;
    this.predicates = [bl.tag_biolink(predicate)];
  ...
  to_trapi() {
    ...
    const constraints = this.get_constraints();
    if (!cmn.is_missing(constraints)) {
      trapi[CONSTANTS.QUALIFIERS.CONSTRAINTS] = constraints;
    }
  ...
  static from_trapi(trapi_qedge, qnodes) {
    ...
    const predicate = cmn.json_get(trapi_qedge, 'predicates')[0];
    const constraints = cmn.json_get(trapi_qedge, 'constraints', []).map(constraint => {
    ...
    const query_mode = cmn.json_get(trapi_qedge, CONSTANTS.QGRAPH.EDGE.QUERY_MODE, CONSTANTS.QGRAPH.LOOKUP);

    return new _QEdge(qnodes[subject], qnodes[object], predicate, constraints, query_mode);
  }
...
class _QEdgeQualifierSet {
  ...
  static from_trapi(trapi_qedge_qualifiers) {
    ...
    const qualifiers = trapi_qedge_qualifiers.map(qualifier => {
      return _QEdgeQualifier.from_trapi(qualifier);
    });
...
class _QEdgeQualifier {
  ...
  static from_trapi(trapi_qedge_qualifier) {
    const type = cmn.json_get(trapi_qedge_qualifier, CONSTANTS.QUALIFIERS.ID);
    const val = cmn.json_get(trapi_qedge_qualifier, CONSTANTS.QUALIFIERS.VALUE);
    return new _QEdgeQualifier(type, val);
  }
}

function _make_qedge_aspect_qualifier(aspect) {
  return new _QEdgeQualifier(bl.tag_biolink('object_aspect_qualifier'), aspect);
}

function _make_qedge_direction_qualifier(direction) {
  return new _QEdgeQualifier(bl.tag_biolink('object_direction_qualifier'), direction);
}

function _make_qedge_predicate_qualifier(pred) {
  return new _QEdgeQualifier(bl.tag_biolink('qualified_predicate'), pred);
}
...
class _QPath {
  ...
  static from_trapi(binding, trapi_qpath) {
    const subject = cmn.json_get(trapi_qpath, 'subject');
    const object = cmn.json_get(trapi_qpath, 'object');
    let constraint = null;
    const constraints = cmn.json_get(trapi_qpath, 'constraints', false);
    if (constraints) {
      constraint = cmn.json_get(constraints, 'intermediate_categories', null);
    }
...
class _QGraph {
  ...
  static from_trapi(trapi_qgraph) {
    ...
    const nodes = cmn.json_get(trapi_qgraph, 'nodes');
    for (const [binding, node] of Object.entries(nodes)) {
      qgraph.qnodes[binding] = _QNode.from_trapi(binding, node);
    }
    const edges = cmn.json_get(trapi_qgraph, 'edges', null);
    const paths = cmn.json_get(trapi_qgraph, 'paths', null);
  ...
  get_type() {
    if (this._query_type === 'pathfinder') return CONSTANTS.QGRAPH.TEMPLATE.PATHFINDER;
    if (this._is_lookup()) return CONSTANTS.QGRAPH.TEMPLATE.LOOKUP;
    ...
    if (start_category === bl.tag_biolink('ChemicalEntity')
        && end_category === bl.tag_biolink('Gene')) {
      return CONSTANTS.QGRAPH.TEMPLATE.CHEMICAL_GENE;
    } else if (start_category === bl.tag_biolink('ChemicalEntity')
        && end_category === bl.tag_biolink('Disease')) {
      return CONSTANTS.QGRAPH.TEMPLATE.CHEMICAL_DISEASE;
    } else if (start_category === bl.tag_biolink('Gene')
        && end_category === bl.tag_biolink('ChemicalEntity')) {
      return CONSTANTS.QGRAPH.TEMPLATE.GENE_CHEMICAL;
    }
  ...
  _is_lookup() {
    const qedge = Object.values(this.qconnections)[0];
    return qedge.query_mode === CONSTANTS.QGRAPH.LOOKUP;
  }
}
