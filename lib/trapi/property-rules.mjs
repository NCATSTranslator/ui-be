export {
  ...
  make_rule_transform_and_rename_attribute_value,
 ...
 * @returns {function} - The extraction rule.
 */
function make_rule_transform_and_rename_attribute_value({attr_id, target_key, transform}) {
  const default_value = null;
  return _make_property_rule({
    key: attr_id,
    transform: (source, attr_id) => {
      const attr_itr = new trapi.AttributeIterator(source);
      const value = trapi.get_attr_val(attr_itr.find_one(attr_id));
