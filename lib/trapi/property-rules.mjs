...
function make_rule_transform_property({source_key, transform}) {
  ...
  return _make_property_rule({
    ...
    update: (target, property) => {
      const current = cmn.json_set_default_and_get(target, source_key, default_value);
      if (cmn.is_missing(current) && !cmn.is_missing(property)) {
        cmn.json_set(target, source_key, property);
      }
      return target;
    },
...
function make_rule_get_property(source_key) {
  return make_rule_transform_property({
    ...
    transform: (source, key) => cmn.json_get(source, key)
  });
}
...
function make_rule_transform_and_aggregate_property({source_key, target_key, transform}) {
  return _make_property_rule({
    ...
    transform: transform,
    update: (target, property) => {
      const current_value = cmn.json_set_default_and_get(target, target_key, []);
...
function make_rule_aggregate_property({source_key, target_key}) {
  return make_rule_transform_and_aggregate_property({
    ...
    transform: (obj) => { return cmn.json_get(obj, source_key); }
  });
}
...
function make_rule_transform_attribute_value({attr_id, target_key, transform, update, default_value}) {
  ...
  if (cmn.is_missing(update)) {
    update = (target, attr) => {
      const current = cmn.json_set_default_and_get(target, target_key, default_value);
      if (cmn.is_missing(current) && !cmn.is_missing(attr)) {
        cmn.json_set(target, target_key, attr);
      }
      return target;
    };
  }
...
function make_rule_map_attributes({attr_ids, target_key, transform, update}) {
  if (cmn.is_missing(update)) {
    update = (target, attrs) => {
      const current = cmn.json_set_default_and_get(target, target_key, []);
      current.push(...attrs);
      return target;
    }
  }
  ...
  return (source, context) => {
    return (target) => {
      try {
        ...
        logger.warn(`Attribute processing error with key: ${key}\n  Error: ${error}`);
        if (context !== undefined) {
          const agent_errors = cmn.json_set_default_and_get(context.errors, context.agent, []);
