...
async function bootstrapConfig(baseFile, overrideFile=null) {
  ...
  if (overrideFile) {
    let overrides = await loadConfigFromFile(overrideFile, config);
...
async function loadConfigFromFile(filePath, baseConfig=null) {
  let config = await readJson(filePath);
  if (!config.document_root && (baseConfig === null || !baseConfig.document_root) {
