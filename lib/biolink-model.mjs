...
export async function loadBiolink(biolinkConfig) {
  ...
  const biolinkModel = await cmn.read_json(`./assets/biolink-model/${biolinkVersion}/biolink-model.json`);
  const slots = cmn.json_get(biolinkModel, 'slots');
  const classes = cmn.json_get(biolinkModel, 'classes');
  BIOLINK_PREDICATES = Object.freeze(makeBlPreds(slots));
  BIOLINK_CLASSES = Object.freeze(makeBlClasses(classes));
  INFORES_CATALOG = Object.freeze(await cmn.read_json(`./assets/biolink-model/common/${inforesCatalog}`));
  PREFIX_CATALOG = Object.freeze(await cmn.read_json(`./assets/biolink-model/common/${prefixCatalog.path}`));
  PREFIX_EXCLUDE_LIST = Object.freeze(prefixCatalog.exclude);
  if (supportDeprecatedPreds) {
    DEPRECATED_TO_QUALIFIED_PREDICATE_MAP = Object.freeze(await cmn.read_json(`./assets/biolink-model/${biolinkVersion}/deprecated-predicate-mapping.json`));
  }
}
...
export function invertBiolinkPred(pred, biolinkify = false) {
  ...
  const biolinkPred= cmn.json_get(BIOLINK_PREDICATES, p, false);
  if (biolinkPred) {
    if (biolinkify) {
...
function distanceFromRelatedTo(slots, pred) {
  for (let level = 0; ;level += 1) {
    ...
    let predObj = cmn.json_get(slots, pred, false);
    if (!predObj) {
      return -1;
    }

    pred = cmn.json_get(predObj, 'is_a', false);
  }
}
...
function makeBlPreds(slots) {
  ...
  Object.keys(blPreds).forEach((pred) => {
      ...
      if (inversePred) {
        const inverseRecord = cmn.json_get(blPreds, inversePred);
...
function getBiolinkPredData(pred) {
  return cmn.json_get(BIOLINK_PREDICATES, sanitizeBiolinkItem(pred), false);
}
...
function makeBlClass(classData) {
  let retval = {};
  retval.is_a = cmn.json_get(classData, 'is_a', null);
  if (retval.is_a) {
    retval.is_a = biolinkifyClass(retval.is_a);
  }
  retval.description = cmn.json_get(classData, 'description', null);
  return retval;
}
...
export function biolinkClassCmpFn(classA, classB) {
  let d1 = cmn.json_get(BIOLINK_CLASSES, classA, false);
  let d2 = cmn.json_get(BIOLINK_CLASSES, classB, false);
