...
export function get_node_type_description(node_type) {
  ...
  const words = sanitize_biolink_item(node_type).split(' ').filter((w) => w !== '');
  if (words.length === 0) return null;
  const node_class = BIOLINK_CLASSES[biolinkify_class(words.join(' '))];
  if (cmn.is_missing(node_class)) return null;
...
export function is_canonical_pred(pred) {
  const p = sanitize_biolink_item(pred);
  const record = cmn.json_get(BIOLINK_PREDICATES, p, false);
  if (!record) {
    throw new Error(`Expected a valid biolink predicate. Got: ${p}`);
  }
  return record.isCanonical;
}
...
export function is_valid_infores(infores) {
  // Object.hasOwn avoids inherited keys (e.g. 'constructor') when ids come from requests.
  return Object.hasOwn(INFORES_CATALOG, infores);
}

export function infores_to_provenance(infores) {
  if (!is_valid_infores(infores)) return null;
  return cmn.deep_copy(INFORES_CATALOG[infores]);
}
...
export function curie_to_normalized_url(curie, curies) {
  if (!curie.startsWith('UMLS')) return curie_to_url(curie);
  curies.sort(_curie_greater_than);
  return curie_to_url(curies[0]);

  function _curie_greater_than(curie1, curie2) {
    const prefix1_ranking = __get_prefix_rank(curie1);
    const prefix2_ranking = __get_prefix_rank(curie2);
    if (prefix1_ranking === prefix2_ranking) return 0;
    if (prefix1_ranking > prefix2_ranking) return -1;
    return 1;

    function __get_prefix_rank(curie) {
      if (!is_valid_curie(curie)) return -1;
      //TODO: Ranking should be configured
...
function InvalidPredicateError(pred) {
  new Error(`Expected a valid biolink predicate. Got: ${pred}`, 'biolink-model.mjs');
}
InvalidPredicateError.prototype = Object.create(Error.prototype);

function InvalidClassError(bl_class) {
  new Error(`Expected a valid biolink class. Got: ${bl_class}`, 'biolink-model.mjs');
}
...
function _biolinkify_pred(pred) {
  const s = pred.replaceAll(' ', '_');
  if (s.startsWith('biolink:')) return s;
  return `biolink:${s}`;
}
