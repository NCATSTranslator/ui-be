...
export function is_base64url_char(ch) {
  return (ch >= 'A' && ch <= 'Z')
    || (ch >= 'a' && ch <= 'z')
    || (ch >= '0' && ch <= '9')
    || ch === '-'
...
export function is_base64url_string(s) {
  if (!is_string(s)) return false;
  for (let i = 0; i < s.length; i++) {
    if (!is_base64url_char(s[i])) return false;
  }
  return true;
}
...
export function distinct_array(a, key_fn = null) {
  if (is_array(a)) {
    if (key_fn === null) return [...new Set(a)];
    const seen = new Map();
    for (const e of a) {
      const key = key_fn(e);
      if (!seen.has(key)) {
...
const _ENTITY_SIG_DOMAIN = 'translator:canvas:entity:v2';
...
export function canonical_json(value) {
  if (is_array(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      const element = canonical_json(value[i]);
      out += (element === undefined) ? 'null' : element;
    }
    return out + ']';
  }
  if (is_object(value)) {
    let out = '{';
    let is_first = true;
    for (const key of _canonical_key_order(value)) {
      const element = canonical_json(value[key]);
      ...
      is_first = false;
      out += JSON.stringify(key) + ':' + element;
    }
    return out + '}';
  }
...
function _canonical_key_order(obj) {
  const index_keys = [];
  const string_keys = [];
  for (const key of Object.keys(obj)) {
    if (_is_array_index_key(key)) {
      index_keys.push(key);
    } else {
      string_keys.push(key);
    }
  }
  index_keys.sort((a, b) => a - b);
  string_keys.sort();
  return index_keys.concat(string_keys);
}

function _is_array_index_key(key) {
  const n = Number(key);
