...
class FakeSessionStore {
  ...
  async updateSession(session, expected_token) {
    await yield_to_event_loop();
    if (!expected_token) {
      throw new Error('updateSession requires the expected current token');
    }
    if (this.row.id !== session.id || this.row.token !== expected_token || this.row.force_kill) {
      return null;
    }
...
async function test_logout_survives_inflight_refresh() {
  const { store, auth_service, controller } = make_fixture(0);
  const req = await make_req(auth_service, store.row.token);
  ast.strictEqual(req.sessionData.status, auth.SESSION_VALID,
    'the in-flight request must start from a live session');

  await auth_service.expireSessionByToken(store.row.token);

  const [success, , errcode] = await controller._refreshSession(req, make_res(), req.sessionData);
  ast.strictEqual(store.row.force_kill, true,
    'an in-flight refresh must not resurrect a logged-out session');
  ast.strictEqual(success, false, 'refreshing a logged-out session must report failure');
  ast.strictEqual(errcode, 401, 'refreshing a logged-out session must map to a 401');
  ast.strictEqual(store.committed_updates, 0, 'a killed session must reject the write');

  const after = await auth_service.getSessionData(store.row.token);
  ast.strictEqual(after.status, auth.SESSION_FORCE_KILLED,
    'the session must still read as force killed after the refresh');
}

async function test_update_session_requires_expected_token() {
  const { store } = make_fixture(0);
  await ast.rejects(() => store.updateSession(new Session({...store.row}), undefined),
    /requires the expected current token/,
