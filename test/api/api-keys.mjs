...
function daysFromNow(iso) {
  return (new Date(iso) - Date.now()) / (24 * 60 * 60 * 1000);
}
...
try {
  ...
  ok(created && Math.abs(daysFromNow(created.time_expires) - 30) < 0.01, 'new key expires 30 days out by default');
} catch (err) {
  fail(`create request failed: ${err.message} -- is the server running with auth_check=false?`);
}

try {
  const requested = isoDaysFromNow(7);
  const { res, json } = await postJson(KEY_PATH, { name: `${KEY_NAME} short-lived`, time_expires: requested });
  ok(res.status === 200, `POST with time_expires responds 200 (got ${res.status})`);
  const shortLived = json && json.api_key;
  ok(shortLived && new Date(shortLived.time_expires).toISOString() === requested, 'honours a requested expiry');
  if (shortLived) await deleteJson(`${KEY_PATH}/${shortLived.id}`);
} catch (err) {
  fail(`short-lived create request failed: ${err.message}`);
}

for (const bad of [isoDaysFromNow(-1), isoDaysFromNow(366), 'soon', 7, null]) {
  try {
    const { res } = await postJson(KEY_PATH, { name: KEY_NAME, time_expires: bad });
    ok(res.status === 400, `POST with time_expires=${JSON.stringify(bad)} responds 400 (got ${res.status})`);
  } catch (err) {
    fail(`bad expiry create request failed: ${err.message}`);
  }
}
...
try {
  const { res } = await postJson('/api/v1/session/status', { update: true }, { Authorization: `Bearer ${rawKey}` });
  ok(res.status === 403, `POST /api/v1/session/status with a key responds 403 (got ${res.status})`);
} catch (err) {
  fail(`key-updating-session request failed: ${err.message}`);
}
